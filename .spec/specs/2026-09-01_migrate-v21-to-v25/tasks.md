...
- [ ] 完成设计方案全链路迁移与 v2.1 功能 parity
  ...
  - verify: v2.1 设计方案导航、列表/详情、创建/编辑/复制/删除、AI 生成/运行、版本与工作台联动按历史实现逐项验证；Web 与 Electron E2E、contracts/repository/API/IPC/features tests 通过。当前本地历史来源 owner-safe 链路、确定性 adapter、固定计划校验、取消和 `.musefold.design` 安全 staging/archive 已接线；本轮 working-tree-only 的 Desktop run adapter 已补 cancellation-wins：部分成功加后续取消时 canonical result、Design Scheme run ledger、terminal event 与 execution registry tombstone 统一为 `cancelled`，所有 active generation job IDs 均 fan-out cancel，并在评估/terminal outward event 前重检重入取消；adapter 定向 suite 16 tests passed。既有 `run-session` 可能已持久化的 evaluation 证据按现有语义保留，本轮未引入删除评价策略；共享 UI、Web runtime、云端 run/assets/package、Desktop Agent event 和真实成功 E2E 尚未闭合
...
- [ ] 运行统一门禁并修复本包发现的可执行问题
  ...
  - verify: `pnpm run check`、`pnpm run test:e2e`、桌面 Electron E2E 和 `check_spec_package.py` 结果真实记录。当前 `pnpm run check` 为 179 个测试文件、1285 个测试、35 个 Turbo task 全绿；本轮正确的根目录 `pnpm run typecheck`、Electron `pnpm run build`、adapter 定向 suite 16 tests 均通过；本轮完整 Electron project 为 32 passed、1 skipped、1 failed，唯一失败仍是原生 macOS fullscreen 的 runner 前台焦点环境阻塞，跳过项为真实 key 场景；此前完整 v25 E2E 为 97 passed、1 failed、5 skipped；Spec 校验仍因未完成 parity/证据门禁失败
