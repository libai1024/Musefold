...
- [ ] 完成设计方案全链路迁移与 v2.1 功能 parity
  ...
  - verify: v2.1 设计方案导航、列表/详情、创建/编辑/复制/删除、AI 生成/运行、版本与工作台联动按历史实现逐项验证；Web 与 Electron E2E、contracts/repository/API/IPC/features tests 通过。当前仅本地持久化、确定性 adapter、固定计划校验、取消和 `.musefold.design` 安全 staging/archive 已接线；另已闭合 owner-safe 历史来源链路：仅接受指定成功且未软删 run 下的 available asset，经受管根校验复制，从 generation ledger 的 immutable prompt snapshot 读取 prompt，并在 domain/source-ingestion 失败时清理 snapshot/package/files；contracts、source-ingestion、domain 3 个定向文件共 87 tests passed。共享 UI、Web runtime、云端 run/assets/package、Desktop Agent event 和真实成功 E2E 尚未闭合
...
- [ ] 运行统一门禁并修复本包发现的可执行问题
  ...
  - verify: `pnpm run check`、`pnpm run test:e2e`、桌面 Electron E2E 和 `check_spec_package.py` 结果真实记录。`pnpm run check` 已通过，完整 v25 E2E 为 94 passed、3 failed、5 skipped（仅原生 fullscreen 环境阻塞及已处理的视觉基线差异），Spec 校验仍因未完成 parity/证据门禁失败
