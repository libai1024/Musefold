...
- [ ] 运行统一门禁并修复本包发现的可执行问题
  ...
  - verify: `pnpm run check`、`pnpm run test:e2e`、桌面 Electron E2E 和 `check_spec_package.py` 结果真实记录。当前 `pnpm run check` 为 180 个测试文件、1301 个测试、35 个 Turbo task 全绿；本轮受影响 Design Scheme 套件 8 个文件、151 个测试通过，根目录 `pnpm run typecheck` 通过；本轮完整 Electron project 为 32 passed、1 skipped、1 failed，唯一失败仍是原生 macOS fullscreen 的 runner 前台焦点环境阻塞，跳过项为真实 key 场景；此前完整 v25 E2E 为 97 passed、1 failed、5 skipped；Spec 校验仍应因未完成 parity/证据门禁失败
