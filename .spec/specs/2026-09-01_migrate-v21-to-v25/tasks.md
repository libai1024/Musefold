...
- [ ] 完成设计方案全链路迁移与 v2.1 功能 parity
  ...
  - verify: v2.1 设计方案导航、列表/详情、创建/编辑/复制/删除、AI 生成/运行、版本与工作台联动按历史实现逐项验证；Web 与 Electron E2E、contracts/repository/API/IPC/features tests 通过。当前本地历史来源 owner-safe 链路、确定性 adapter、固定计划校验、取消和 `.musefold.design` 安全 staging/archive 已接线；共享屏已增加可选详情生命周期回调，Web `/design-schemes` 已同步 `?scheme=<id>` deep-link、返回/删除清理 query，并拒绝非法 scheme 参数，正式方案详情已补齐常驻「在 Composer 中修改」入口并保留下拉菜单补充动作，相册键盘与横向触控导航已补齐，相关 Web/features 定向测试通过。共享 UI、Web runtime、云端 run/assets/package、Desktop Agent event 和真实成功 E2E 尚未闭合
