...
function memoryKeychain(): AiSecretKeychain & {
...
function fakeClient(overrides: Partial<NewApiClient> = {}): NewApiClient {
  ...
  return {
    ...
    listTokens: vi.fn(async () => [
      { id: 9, name: "musefold-test-abcd", status: 1, keyMasked: "tP1**" },
    ]),
    ...
    getNotices: vi.fn(async () => [
      { id: "n-1", content: "公告", publishedAt: null },
    ]),
    ...overrides,
  };
}
...
describe("login 编排（§5.1）", () => {
  it("成功路径：refresh 入 keychain、双栈供给、会话与后 4 位落库、广播", async () => {
    const h = makeHarness();
    const status = await h.service.login({
      username: "wang",
      password: "pw-secret",
    });
    ...
    // 定价同步（version 门控首轮）：0.04 × 10 × 1 = 0.4 积分/张
    expect(h.provisioner.priceCalls).toEqual([
      { providerId: "prov-1", pricePoints: 0.4 },
    ]);
  ...
  it("只向主进程提供官方 Cloud 身份，并复用受控的管理 JWT 续期", async () => {
    const h = makeHarness();
    await h.service.login({ username: "wang", password: "pw" });
    expect(h.service.cloudIdentity()).toEqual({
      ownerId: "4",
      username: "wang",
      cloudBaseUrl: `${DEFAULT_ACCOUNT_SERVER_URL}/api/musefold/v1`,
    });
    await expect(h.service.managementAccessToken()).resolves.toBe("jwt-1");

    nowMs += 1_800_000;
    await expect(h.service.managementAccessToken()).resolves.toBe("jwt-2");
  ...
  it("新设备：createToken → listTokens 按名定位 → fetchTokenKey", async () => {
    const h = makeHarness();
    await h.service.login({ username: "wang", password: "pw" });
    expect(h.client.createToken).toHaveBeenCalledWith("jwt-1", {
      name: "musefold-test-abcd",
    });
  ...
  it("令牌在网页端被删：取回失败 → 自动新建（自愈）", async () => {
    ...
    fetchKey.mockRejectedValueOnce(
      new RelayApiError("ACCOUNT/SERVER", "令牌不存在", 404),
    );
    await h.service.login({ username: "wang", password: "pw" });
  ...
  it("编排中途失败 → 全量回滚：keychain 清空、session 置空、错误带阶段", async () => {
    ...
    const error = await h.service
      .login({ username: "wang", password: "pw" })
      .then(
        () => null,
        (e: unknown) => e,
      );
  ...
  it("register 成功后自动登录（US-01）", async () => {
    const h = makeHarness();
    const status = await h.service.register({
      username: "wang",
      password: "pw",
    });
...
describe("ensureJwt 静默续期（§5.2）", () => {
  ...
  it("服务器不可达 → health=unreachable，余额缓存保留", async () => {
    ...
    nowMs += 1800_000;
    await expect(h.service.refreshQuota()).rejects.toMatchObject({
      code: "ACCOUNT/NETWORK",
    });
    const status = h.service.status();
...
describe("refreshQuota 与定价门控（FR-GW-09）", () => {
  ...
  it("version 变化 → 重新应用单价（含分组倍率）", async () => {
    ...
    await h.service.refreshQuota();
    expect(h.provisioner.priceCalls[1]).toEqual({
      providerId: "prov-1",
      pricePoints: 1,
    });
  });
...
describe("redeem（FR-ACC-05）", () => {
  ...
  it("空码本地拦截 → REDEEM_INVALID；未登录 → AUTH", async () => {
    const h = makeHarness();
    await expect(h.service.redeem("x")).rejects.toMatchObject({
      code: "ACCOUNT/AUTH",
    });
    await h.service.login({ username: "wang", password: "pw" });
    await expect(h.service.redeem("   ")).rejects.toMatchObject({
      code: "ACCOUNT/REDEEM_INVALID",
    });
  });
});
...
describe("setServerUrl（FR-ACC-08）", () => {
  it("已登录拒绝；未登录归一化保存", async () => {
    ...
    await h.service.login({ username: "wang", password: "pw" });
    await expect(
      h.service.setServerUrl("https://x.test"),
    ...
    await h.service.login({ username: "wang", password: "pw" });
    expect(h.service.cloudIdentity()).toBeNull();
  });
});
...
describe("官方入口故障切换", () => {
  it("主域名网络失败时切到 IP，并让托管连接使用实际入口", async () => {
    ...
    expect(provisioner.upsertCalls[0]).toMatchObject({
      serverUrl: DEFAULT_ACCOUNT_SERVER_FALLBACK_URL,
    });
