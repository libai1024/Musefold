...
import { ACCOUNT_QUOTA_PER_POINT, ACCOUNT_QUOTA_PER_USD } from '@musefold/contracts/billing.js';
  ...
  private async syncPricingAndNotices(): Promise<void> {
    ...
    if (pricing.version && pricing.version !== session.pricingVersion) {
      ...
      if (image && session.managedProviderId) {
        ...
        const pricePoints =
          image.modelPrice * (ACCOUNT_QUOTA_PER_USD / ACCOUNT_QUOTA_PER_POINT) * (groupRatio || 1);
