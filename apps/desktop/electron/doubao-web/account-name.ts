...
const RESERVED_LABELS =
  /^(登录|设置|更多|搜索|新对话|新工作任务|技能|AI 创作|云盘|主对话|下载电脑版)$/i;
...
function candidateLabel(candidate: DoubaoAccountNameCandidate): string | null {
  ...
  const label = lines.find(
    (line) => line.length <= 32 && !RESERVED_LABELS.test(line) && !/^\d+$/.test(line),
  );
  return label ?? null;
}
 ...
 */
export function pickDoubaoAccount(
  candidates: DoubaoAccountNameCandidate[],
): DoubaoAccountIdentity | null {
...
export function isAllowedDoubaoAvatarUrl(value: string): boolean {
  try {
    ...
    return ['byteacctimg.com', 'byteimg.com', 'doubao.com', 'doubao.cn'].some(
