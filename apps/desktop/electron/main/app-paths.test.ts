...
describe('resolveAppRoot', () => {
  ...
  it('walks up from apps/desktop/out/main to the app root when unpackaged', () => {
    const repoRoot = resolve('/repo');
    const appPath = join(repoRoot, 'apps', 'desktop', 'out', 'main');
  ...
  it('falls back to cwd when unpackaged appPath does not contain the app root', () => {
    ...
    expect(
      resolveAppRoot(
        env({
          packaged: false,
          appPath: join(resolve('/tmp/unrelated'), 'apps', 'desktop', 'out', 'main'),
          cwd,
        }),
        probeFor(cwd),
      ),
  ...
  it('falls back to appPath without throwing when neither walk nor cwd matches', () => {
    const appPath = join(resolve('/tmp/unrelated'), 'apps', 'desktop', 'out', 'main');
...
describe('resolveResourcePath', () => {
  ...
  it('joins <appRoot>/resources when unpackaged', () => {
    ...
    const unpackaged = env({
      packaged: false,
      appPath: join(repoRoot, 'apps', 'desktop', 'out', 'main'),
      cwd: resolve('/elsewhere'),
      resourcesPath: resolve('/packaged/Resources'),
    });
