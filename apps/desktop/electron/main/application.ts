...
app.on("child-process-gone", (_event, details) => {
  ...
  reportMainDiagnostic(
    new Error(`Electron 子进程已退出：${details.type}/${details.reason}`),
    {
      source: "main-process",
...
app.whenReady().then(async () => {
  ...
  const ownership = await acquireDesktopOwnerLockWithHeadlessTakeover(
...
function openMainWindow(): void {
  ...
  showMainWindow(main && !main.isDestroyed() ? main : createMainWindow());
}
