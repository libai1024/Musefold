...
export function registerCloudSyncHandlers(dependencies: CloudSyncHandlerDependencies = {}): void {
  ...
  target.handle(IPC.CLOUD_SYNC_CONFLICTS, () => invokeCloudSync(() => service.listConflicts()));
  target.handle(IPC.CLOUD_SYNC_RESOLVE, (_event, conflictId: unknown, resolution: unknown) => {
    if (typeof conflictId !== 'string' || !conflictId.trim()) throw new Error('同步冲突标识无效');
  ...
  target.handle(IPC.CLOUD_CONNECTIONS_LIST, () => invokeCloudSync(() => service.listConnections()));
  target.handle(IPC.CLOUD_CONNECTIONS_UPDATE, (_event, id: unknown, input: unknown) =>
    invokeCloudSync(() =>
...
function requiredConnectionId(value: unknown): string {
  if (typeof value !== 'string' || !value.trim()) throw new Error('Cloud MCP 连接标识无效');
  return value.trim();
}
