...
export function registerDesignSchemeHandlers(): void {
  ipcMain.handle(
    IPC.DESIGN_SCHEME_CREATE_START,
    async (
      event,
      ...
      if (
        !request.brief?.trim() &&
        !request.githubUrl &&
        !request.githubUrls?.length &&
        !request.history?.items?.length
      ) {
        return fail(
          appError('REQUIRED', '请描述你的方案想法，或提供一个 GitHub Skill 地址', {
      ...
      if (sessions.has(request.executionId)) {
        return fail(
          appError('INVALID_STATE', '相同的创建任务正在进行中', { recoveryAction: 'retry' }),
        );
      }
  ...
  ipcMain.handle(
    ...
    (_event, executionId: string, accept: boolean) => {
  ...
  ipcMain.handle(IPC.DESIGN_SCHEME_LIST, (): AppResult<DesignSchemeSummary[]> => {
    try {
      ...
      return fail(
        appError('UNKNOWN', '读取设计方案列表失败', { retryable: true, recoveryAction: 'retry' }),
      );
    }
  });

  ipcMain.handle(
    IPC.DESIGN_SCHEME_LIST_ASSETS,
    (_event, schemeId: string): AppResult<DesignSchemeAssetSummary[]> => {
      try {
        const repository = new DesignSchemeRepository(getDesignSchemeDb());
        ...
        return fail(
          appError('UNKNOWN', '读取方案相册失败', { retryable: true, recoveryAction: 'retry' }),
        );
      }
    },
  );
  ...
  ipcMain.handle(
    ...
    (
      ...
      try {
        const repository = new DesignSchemeRepository(getDesignSchemeDb());
        const sanitized = inputs.map((slot) => ({
          id: String(slot.id),
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_GET_REVISION,
    (_event, revisionId: string): AppResult<DesignSchemeRevisionDocument> => {
      try {
        const repository = new DesignSchemeRepository(getDesignSchemeDb());
        ...
        return fail(
          appError('UNKNOWN', '读取设计方案版本失败', { retryable: true, recoveryAction: 'retry' }),
        );
      }
    },
  );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (
        ...
        !request.executionId ||
        typeof request.schemeId !== 'string' ||
      ...
      if (runExecutions.has(request.executionId)) {
        return fail(
          appError('INVALID_STATE', '相同的运行任务正在进行中', { recoveryAction: 'retry' }),
        );
      }
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_SELECT_COVER,
    (_event, schemeId: string, assetId: string): AppResult<DesignSchemeSummary> => {
      try {
        const repository = new DesignSchemeRepository(getDesignSchemeDb());
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_FORMALIZE,
    (_event, schemeId: string): AppResult<DesignSchemeSummary> => {
      try {
        const repository = new DesignSchemeRepository(getDesignSchemeDb());
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_RENAME,
    (_event, schemeId: string, name: string): AppResult<DesignSchemeSummary> => {
  ...
  ipcMain.handle(IPC.DESIGN_SCHEME_REMOVE, (_event, schemeId: string): AppResult<{ ok: true }> => {
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_LIST_SOURCE_FILES,
    (_event, schemeId: string): AppResult<DesignSchemeSourceSnapshotDetail[]> => {
      try {
        const repository = new DesignSchemeRepository(getDesignSchemeDb());
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_MODIFY_START,
    async (
      event,
      ...
      if (
        ...
        !request.executionId ||
        typeof request.schemeId !== 'string' ||
        typeof request.baseRevisionId !== 'string'
      ) {
      ...
      if (modifySessions.has(request.executionId)) {
        return fail(
          appError('INVALID_STATE', '相同的修改任务正在进行中', { recoveryAction: 'retry' }),
        );
      }
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_PROMOTE_DRAFT,
    (_event, schemeId: string): AppResult<DesignSchemeSummary> => {
      try {
        const repository = new DesignSchemeRepository(getDesignSchemeDb());
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_CHECK_UPDATE,
    async (_event, schemeId: string): Promise<AppResult<DesignSchemeCheckUpdateResult>> => {
      if (typeof schemeId !== 'string' || !schemeId) {
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_MARKET_SEARCH,
    async (_event, query: string): Promise<AppResult<MarketSearchResult>> => {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (!targetPath) {
        ...
        const safeName = (summary?.name ?? 'design-scheme')
          .replace(/[\\/:*?"<>|\s]+/g, '-')
          .slice(0, 60);
  ...
  ipcMain.handle(
    IPC.DESIGN_SCHEME_IMPORT,
    async (event, explicitSource?: string): Promise<AppResult<DesignSchemeImportOutcome>> => {
