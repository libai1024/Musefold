...
export function registerFolderHandlers(): void {
  ...
  ipcMain.handle(IPC.FOLDERS_CREATE, (_e, f) => {
    const value = foldersRepo.create(f);
    scheduleCloudSync();
    return value;
  });
  ipcMain.handle(IPC.FOLDERS_UPDATE, (_e, id: string, patch) => {
    const value = foldersRepo.update(id, patch);
    scheduleCloudSync();
    return value;
  });
  ipcMain.handle(IPC.FOLDERS_DELETE, (_e, id: string) => {
    foldersRepo.delete(id);
    scheduleCloudSync();
    return { ok: true as const };
  });
  ipcMain.handle(IPC.FOLDERS_REORDER, (_e, ids: string[]) => {
