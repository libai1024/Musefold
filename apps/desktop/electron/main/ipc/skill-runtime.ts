...
function imageRoleContract(images: readonly AiSkillImageReference[]): string {
  ...
  const list = images.map(
    (image) =>
      `图 ${image.index}（${image.name}）：${
        image.role === 'content-source'
...
async function stageRuntimeImages(
  ...
  const slots = Math.max(
    0,
    Math.min(MAX_SKILL_STYLE_REFERENCES, MAX_REFERENCE_IMAGES, Math.floor(availableSlots)),
  );
  if (slots === 0) return [];
  const images = runtimeFiles(record)
    ...
    .sort((left, right) => {
      ...
      return (
        score(left.relativePath) - score(right.relativePath) ||
...
function invalidRuntime(message: string): AppResult<never> {
  return fail(
    appError('MISSING_REFERENCE', message, { retryable: false, recoveryAction: 'retry' }),
  );
}
...
async function runSkillAgent(
  ...
  const tools = {
    ...
    read_skill_file: tool({
      ...
      execute: async ({ path }: { path: string }) => {
        ...
        if (readBudget.remaining <= 0)
          throw new Error('Skill 文本读取额度已用完，请直接给出最终提示词');
    ...
    generate_image: tool({
      description:
        '用最终生图提示词调用图像模型出图。整个任务只能调用一次；画面比例、张数与模型由应用设置决定。',
      inputSchema: z.object({
        prompt: z
          .string()
          .min(1)
          .describe('可直接交给生图模型的完整最终提示词；若有用户图片必须逐一说明「图 N」的角色'),
      }),
      execute: async ({ prompt }: { prompt: string }) => {
        ...
        const succeeded = generation.outcomes.filter(
        ...
        return {
          ...
          ...(succeeded === 0
            ? {
                error:
                  generation.outcomes.find((outcome) => outcome.result.error)?.result.error
                    ?.message ?? '图片生成失败',
              }
            : {}),
        };
  ...
  if (ctx.signal.aborted) {
    const outcomes = generation
      ? (generation as { outcomes: SkillRuntimeGenerationOutcome[] }).outcomes
      : cancelledOutcomes(ctx);
    ctx.trace.upsert({
      id: 'agent-run',
      kind: 'tool',
      title: 'Agent 执行 Skill',
...
function validTraceSeed(seed: unknown): SkillRuntimeTraceItem[] {
  ...
  return seed.filter((item): item is SkillRuntimeTraceItem =>
    Boolean(
      item &&
        typeof item === 'object' &&
...
export async function prepareGithubSkillRuntime(
  ...
  const textNames = (result.data.runtimeFiles ?? [])
    .filter(
      (file) => TEXT_FILE_PATTERN.test(file.relativePath) && utf8RuntimeText(file.bytes) !== null,
    )
    .map((file) => file.relativePath);
  const imageNames = files
    .filter((file) => file.fileKind === 'asset')
    .map((file) => file.relativePath);
...
export async function executeSkillRuntime(
  ...
  if (files.length === 0) {
    return fail(
      appError('MISSING_REFERENCE', 'Skill 中没有可读取的 Markdown 或 TXT 文件', {
        retryable: false,
  ...
  const executionId =
    typeof request.executionId === 'string' && request.executionId ? request.executionId : ulid();
  ...
  try {
    ...
    const providerRow = getDb()
      ...
      .get(providerId) as { type: ProviderType } | undefined;
