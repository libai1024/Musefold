...
// 避免主窗口与宠物窗口落到不同根目录。
//
// HOT-08 之前候选读取器默认返回空列表，一律回落到随包内置 apps/desktop/out/renderer。
 ...
 *  打包/未打包同一条规则：asar 内保持 apps/desktop/out/**，不做 from/to 重映射。 */
export function getBuiltinRendererRoot(): string {
  return join(getAppRoot(), 'apps/desktop/out/renderer');
}
