...
describe('local Agent Skill filesystem limits', () => {
  it.runIf(process.platform !== 'win32')(
    'rejects symlinks without following them outside the selected root',
    async () => {
      ...
      const result = await readLocalAgentSkillSource({
  ...
  it('rejects a text file above the per-file limit before loading it into a snapshot', async () => {
    ...
    mkdirSync(join(root, 'references'));
    writeFileSync(
      join(root, 'references', 'too-large.md'),
      Buffer.alloc(SKILL_MAX_TEXT_FILE_BYTES + 1, 65),
    );
  ...
  it('rejects more than the allowed number of text files', async () => {
    ...
    for (let index = 0; index < SKILL_MAX_TEXT_FILES; index += 1) {
      writeFileSync(
        join(root, 'references', `rule-${String(index).padStart(3, '0')}.md`),
        `Rule ${index}`,
      );
    }
