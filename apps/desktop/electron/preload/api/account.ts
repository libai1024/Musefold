...
async function invokeCloudSync<T>(channel: string, ...args: unknown[]): Promise<T> {
  try {
    return (await ipcRenderer.invoke(channel, ...args)) as T;
  } catch (error) {
    const raw = error instanceof Error ? error.message : String(error);
...
export const accountApi = {
  status: () =>
    invokeAccount<import('@musefold/desktop-contracts/account').AccountStatus>(IPC.ACCOUNT_STATUS),
  register: (input: import('@musefold/desktop-contracts/account').AccountCredentialsInput) =>
  ...
  logout: () =>
    invokeAccount<import('@musefold/desktop-contracts/account').AccountStatus>(IPC.ACCOUNT_LOGOUT),
  redeem: (code: string) =>
...
export const cloudSyncApi = {
  ...
  conflicts: () =>
    invokeCloudSync<import('@musefold/desktop-contracts/cloud-sync').CloudSyncConflictSummary[]>(
...
export const cloudConnectionsApi = {
  list: () =>
    invokeCloudSync<import('@musefold/contracts').McpConnectionPage>(IPC.CLOUD_CONNECTIONS_LIST),
  update: (id: string, input: import('@musefold/contracts').UpdateMcpConnection) =>
    invokeCloudSync<import('@musefold/contracts').McpConnectionPage>(
      IPC.CLOUD_CONNECTIONS_UPDATE,
      id,
      input,
    ),
  revoke: (id: string) => invokeCloudSync<void>(IPC.CLOUD_CONNECTIONS_REVOKE, id),
};
