...
export const designSchemeApi = {
  ...
  cancelRun: (executionId: string) => ipcRenderer.invoke(IPC.DESIGN_SCHEME_RUN_CANCEL, executionId),
  selectCover: (schemeId: string, assetId: string) =>
    ipcRenderer.invoke(IPC.DESIGN_SCHEME_SELECT_COVER, schemeId, assetId),
  formalize: (schemeId: string) => ipcRenderer.invoke(IPC.DESIGN_SCHEME_FORMALIZE, schemeId),
  rename: (schemeId: string, name: string) =>
    ipcRenderer.invoke(IPC.DESIGN_SCHEME_RENAME, schemeId, name),
  remove: (schemeId: string) => ipcRenderer.invoke(IPC.DESIGN_SCHEME_REMOVE, schemeId),
  ...
  checkUpdate: (schemeId: string) => ipcRenderer.invoke(IPC.DESIGN_SCHEME_CHECK_UPDATE, schemeId),
  marketSearch: (query: string) => ipcRenderer.invoke(IPC.DESIGN_SCHEME_MARKET_SEARCH, query),
  exportScheme: (schemeId: string, targetPath?: string) =>
    ipcRenderer.invoke(IPC.DESIGN_SCHEME_EXPORT, schemeId, targetPath),
  importScheme: (sourcePath?: string) => ipcRenderer.invoke(IPC.DESIGN_SCHEME_IMPORT, sourcePath),
  onEvent: (
    cb: (
