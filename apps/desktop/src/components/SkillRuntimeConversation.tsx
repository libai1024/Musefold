...
export function SkillRuntimeConversation({
  trace,
  runningLabel = '正在执行 Skill',
  doneLabel = '已完成 Skill 调用',
}: {
  ...
  return (
    <div className="mb-3" data-testid="skill-runtime-conversation" data-placement="conversation">
      ...
      >
        ...
        <ChevronDown
          className={cn('h-3.5 w-3.5 text-tertiary transition-transform', expanded && 'rotate-180')}
        />
      </button>
      {expanded && (
              ...
              <div className="min-w-0 pl-0.5">
                <div className="flex items-center gap-2 text-meta text-primary">
                  ...
                  {typeof item.durationMs === 'number' && (
                    <span className="text-meta text-quaternary">{item.durationMs}ms</span>
                  )}
                  <span className="ml-auto text-meta text-quaternary">
                    {item.kind === 'assistant'
                      ? item.title.includes('Agent')
                        ? 'Agent 返回'
                        : '转发提示词'
                      : item.kind === 'tool'
                        ? '工具'
                ...
                {item.detail && (
                  <p className="mt-0.5 break-words text-meta leading-relaxed text-tertiary">
                    {item.detail}
                  </p>
                )}
                {item.kind === 'assistant' && item.title === 'Agent' ? (
                  ...
                  (item.output || item.status === 'running') && (
                    <p
                      className="mt-1 max-h-52 overflow-y-auto whitespace-pre-wrap break-words text-meta leading-relaxed text-secondary"
                      data-testid="skill-runtime-agent-output"
                    >
                      {item.output}
                      {item.status === 'running' && (
                        <span
                  ...
                  <details className="mt-1.5 border-l-2 border-accent/35 pl-2" open>
                    <summary className="cursor-pointer text-meta font-medium text-secondary">
                      查看完整返回内容
                    </summary>
                    <p
                      className="mt-1.5 max-h-28 overflow-y-auto whitespace-pre-wrap break-words text-meta leading-relaxed text-secondary"
                      data-testid="skill-runtime-agent-output"
                    >
                      {item.output}
                    </p>
