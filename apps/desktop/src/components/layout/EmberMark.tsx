...
import { capturePageSelection, createSlip, recallSlip, type CapturedSelection } from './emberSlips';
...
export function EmberMark() {
  ...
  const skillBusy = useSkillRuntimeStore(
    (s) => s.status === 'detecting' || s.status === 'executing',
  );
  ...
  const [annotation, setAnnotation] = useState<{ slipId: string | null; label: string } | null>(
    null,
  );
  ...
  useGSAP(
    (_context, contextSafe) => {
      ...
      if (reducedMotion === 'off') cleanup = bindMotion();
        ...
        media.add('(prefers-reduced-motion: no-preference)', bindMotion);
    ...
    <div className={cn('absolute right-5 top-5 z-30', hatchPhase === 'flying' && 'opacity-0')}>
      ...
      >
        ...
        <span
          ref={ringARef}
          aria-hidden="true"
          className="pointer-events-none absolute h-4 w-4 rounded-full border border-accent opacity-0"
        />
        <span
          ref={ringBRef}
          aria-hidden="true"
          className="pointer-events-none absolute h-4 w-4 rounded-full border-[1.5px] border-accent opacity-0"
        />
