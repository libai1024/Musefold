...
export function TitleBar() {
  ...
  const shortTitle = productTopbarDisplayTitle(fullTitle, currentView === 'generate' ? 16 : 10);
  const taskSummary =
    currentView === 'generate' && activeSessionId ? summarizeWorkbenchTask(turns) : null;
  ...
  const sessionMenu =
    currentView === 'generate' && activeSessionId ? (
      <WorkbenchSessionMenuTrigger
        title={sessionTitle}
        pinned={isPinned}
        onTogglePinned={() => setPinnedSessionIds(setSessionPinned(activeSessionId, !isPinned))}
        onRename={openRename}
  ...
  const leading = sidebarCollapsed ? (
    <div className="no-drag" style={{ marginLeft: Math.max(0, leftInset - 12) }}>
      <IconButton
  ...
  const titleSuffix =
    currentView === 'generate' && activeSessionId ? (
      <>
        {taskSummary ? (
          <div className="mf-product-topbar-task-meta" data-testid="titlebar-task-summary">
            <span data-testid="titlebar-result-summary">{taskSummary.activityLabel}</span>
