...
function TypewriterText({ text, testId }: { text: string; testId: string }) {
  ...
  return (
    <p
      className="whitespace-pre-wrap break-words text-[12px] leading-relaxed text-primary"
      data-testid={testId}
      data-complete={done}
    >
      ...
      {!done && (
        <span
          className="ml-0.5 inline-block h-3 w-[5px] animate-pulse rounded-[1px] bg-accent align-middle"
          aria-hidden
        />
      )}
    </p>
  ...
  return (
    <div data-testid="scheme-creation-conversation" data-state={source.state}>
      ...
      {confirmation && running && (
        <div
          className="mb-3 max-w-[440px] rounded-lg border border-accent/35 bg-popover p-4"
          data-testid="scheme-creation-confirm"
        >
          ...
          <div className="mt-3 space-y-1.5 border-y border-border-subtle py-2.5 text-meta text-secondary">
            <p className="flex items-center gap-2">
              ...
              {confirmation.textFileCount} 个文本文件 · {confirmation.imageFileCount} 张参考图（固定
              commit {confirmation.commitHash?.slice(0, 10) ?? confirmation.resolvedRef}）
            </p>
            <p className="flex items-center gap-2">
          ...
          <div className="mt-3 flex justify-end gap-2">
            <button
              ...
              data-testid="scheme-creation-confirm-reject"
            >
              取消创建
            </button>
            <button
              ...
              data-testid="scheme-creation-confirm-accept"
            >
      ...
      {source.error && !running && source.state !== 'cancelled' && (
        <p className="mb-3 text-[11px] text-danger" data-testid="scheme-creation-error">
          {source.error}
        </p>
      )}
      {source.state === 'cancelled' && (
        <p className="mb-3 text-[11px] text-tertiary" data-testid="scheme-creation-cancelled">
          创建已取消，Composer 内容未丢失。
        </p>
      )}
      ...
      {draft && (
        <div className="space-y-3">
          ...
          <div
            className="max-w-[440px] rounded-lg border border-border-default bg-popover p-4"
            data-testid="scheme-creation-draft-card"
          >
            <div className="flex items-start gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="shrink-0 rounded-full border border-border-subtle px-1.5 py-0.5 text-meta text-secondary">
                    {FIDELITY_LABEL[draft.fidelity] ?? draft.fidelity}
                  </span>
                </div>
                ...
                {draft.inputLabels.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {draft.inputLabels.map((label) => (
                      <span
                        key={label}
                        className="rounded-md border border-border-subtle bg-elevated px-1.5 py-0.5 text-meta text-secondary"
                      >
                        {label}
                      </span>
                    ))}
