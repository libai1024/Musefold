...
export function SourceSnapshotDialog({
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[min(600px,88dvh)] w-full max-w-[560px] flex-col rounded-lg border border-border-default bg-popover shadow-pop animate-dialog-in"
        role="dialog"
        aria-modal="true"
        aria-labelledby="scheme-source-title"
      >
        <div className="flex items-start gap-3 border-b border-border-subtle px-5 py-4">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="scheme-source-title" className="text-[14px] font-semibold text-primary">
              方案来源
            </h2>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 text-[11px] leading-6 text-secondary">
          {error ? (
            ...
            <div className="flex items-center justify-center gap-2 py-10 text-tertiary">
            ...
            <div className="space-y-5">
              {briefExcerpt && (
                <div>
                  ...
                  <p className="mt-1 whitespace-pre-wrap rounded-md bg-inset/55 px-3 py-2 text-meta leading-5">
                    {briefExcerpt}
                  </p>
                </div>
              )}
                ...
                >
                  <p className="flex flex-wrap items-center gap-x-2 gap-y-0.5 font-medium text-primary">
                    ...
                    {snapshot.repositoryUrl && (
                      <button
                        type="button"
                        onClick={() =>
                          window.open(snapshot.repositoryUrl!, '_blank', 'noopener,noreferrer')
                        }
                        className="inline-flex items-center gap-1 text-meta font-normal text-primary hover:underline"
                      >
                        <ExternalLink className="h-3 w-3" />
                        {snapshot.repositoryUrl.replace(/^https:\/\/github\.com\//, '')}
                      </button>
                    )}
                  </p>
                  <p className="mt-0.5 text-meta text-tertiary">
                    {snapshot.commitHash
                      ? `commit ${snapshot.commitHash.slice(0, 10)}`
                      : `引用 ${snapshot.ref}`}
                  ...
                  <ul className="mt-2 space-y-0.5 border-l border-border-subtle pl-3">
                      ...
                      <li key={file.path} className="text-meta">
                        {file.kind === 'text' && file.textExcerpt ? (
                          <details>
                            ...
                            <pre className="mt-1 max-h-44 overflow-y-auto whitespace-pre-wrap rounded-md bg-inset/55 px-2.5 py-2 font-mono text-meta leading-4 text-secondary">
                              {file.textExcerpt}
                            </pre>
                          </details>
                        ) : (
                          <span className="flex items-center gap-1.5">
                            {file.kind === 'image' ? (
                              <FileImage className="h-3 w-3 shrink-0 text-tertiary" />
                            ) : (
                              <File className="h-3 w-3 shrink-0 text-tertiary" />
                            )}
                            <span className="min-w-0 truncate text-primary">{file.path}</span>
                            <span className="shrink-0 text-tertiary">
                              {formatBytes(file.sizeBytes)}
                    ...
                    {snapshot.files.length === 0 && (
                      <li className="text-meta text-tertiary">快照内没有文件</li>
                    )}
                  </ul>
                </div>
              ))}
...
export function RenameDialog({
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center bg-black/35 p-4 animate-overlay-in"
      data-testid="scheme-rename-dialog"
    >
      <div
        className="w-full max-w-[400px] rounded-lg border border-border-default bg-popover shadow-pop animate-dialog-in"
        role="dialog"
        aria-modal="true"
        aria-labelledby="scheme-rename-title"
      >
        <div className="flex items-center justify-between gap-3 border-b border-border-subtle px-5 py-4">
          <h2 id="scheme-rename-title" className="text-[14px] font-semibold text-primary">
            重命名方案
          </h2>
          <button
            type="button"
...
export function RemoveDialog({
  ...
  return (
    <div
      className="fixed inset-0 z-[120] flex items-center justify-center bg-black/35 p-4 animate-overlay-in"
      data-testid="scheme-remove-dialog"
    >
      <div
        className="w-full max-w-[400px] rounded-lg border border-border-default bg-popover shadow-pop animate-dialog-in"
        role="dialog"
        aria-modal="true"
        aria-labelledby="scheme-remove-title"
      >
