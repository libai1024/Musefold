...
export function SkillRuntimeAttachment() {
  ...
  const activeProvider = useGenerationStore(
    (state) =>
  ...
  const repository =
    sourceUrl?.replace(/^https?:\/\/github\.com\//, '').replace(/\/$/, '') ?? 'GitHub Skill';
  ...
  return (
    <div
      className="flex h-12 min-w-[264px] max-w-[360px] items-center gap-2.5 rounded-lg border border-accent/35 bg-popover px-2.5"
      data-testid="skill-runtime-chip"
      data-status={status}
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-meta font-medium text-primary">
          {attachment?.name || repository}
        </span>
        <span
          className={cn(
            'mt-0.5 flex min-w-0 items-center gap-1 text-meta',
