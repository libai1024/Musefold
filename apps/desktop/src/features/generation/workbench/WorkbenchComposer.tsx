...
import { composerPresentationMode, composerPresentationModeLocked } from './composerPresentation';
...
export function WorkbenchComposer({
  ...
  const importGithubFromClipboard = async () => {
    ...
    toast.show({
      ...
      description: '把公开仓库、Skill 目录或 SKILL.md 地址直接粘贴到 Composer。',
    });
  };
  const startDesignCreation = async (seed: string, explicitGithubUrls?: string[]) => {
    ...
    const chipUrl =
      skillRuntimeStatus === 'ready' ? (skillRuntimeSourceUrl ?? undefined) : undefined;
    const inlineUrls = seed.match(/https:\/\/github\.com\/[^\s]+/gi) ?? [];
    // 多个地址合并编译为一个组合方案（P3）；去重保持出现顺序。
    const sourceUrls = [
      ...new Set([...(explicitGithubUrls ?? []), ...(chipUrl ? [chipUrl] : []), ...inlineUrls]),
    ];
    ...
    if (!brief && sourceUrls.length === 0 && !history?.items.length) {
      toast.show({
        title: '先描述你的方案想法',
        description: '输入一段话，或粘贴一个 GitHub Skill 地址，再创建设计方案。',
      });
      textareaRef.current?.focus();
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (skillRuntimeAttachment && skillRuntimeStatus === 'ready' && submissionProvider) {
      ...
      if (!execution) {
        toast.error(
          'Skill 执行失败',
          useSkillRuntimeStore.getState().error || '请重新粘贴 Skill 地址后再试',
        );
        return;
      }
  ...
  const plainSource =
    !refinementContext && !schemeSource && source.kind !== 'manual' ? source : null;
