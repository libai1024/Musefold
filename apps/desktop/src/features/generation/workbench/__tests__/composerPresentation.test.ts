...
describe('composer presentation mode', () => {
  ...
  it('prioritizes locked runtime contexts', () => {
    expect(composerPresentationMode({ ...base, designPlanIntent: { prompt: 'x' } })).toBe(
      'design-plan',
    );
    expect(composerPresentationMode({ ...base, skillRuntimeStatus: 'ready' })).toBe('skill');
    expect(composerPresentationMode({ ...base, schemeSource: { mode: 'trial' } })).toBe('scheme');
    expect(composerPresentationMode({ ...base, refinementContext: {} })).toBe('refinement');
