...
describe('workbench image and message interaction contract', () => {
  ...
  it('keeps referenced images inspectable and user edits explicit', () => {
    ...
    expect(sharedUserMessage).toContain('data-testid="generation-user-message-actions"');
  ...
  it('keeps the refinement target above the composer and the surface minimal', () => {
    ...
    const composerSurface = workbench.indexOf('surfaceRef={composerSurfaceRef}');
    expect(attachmentSlot).toBeGreaterThan(-1);
    expect(attachmentSlot).toBeLessThan(composerSurface);
    expect(workbench.indexOf('<RefinementTargetReference')).toBeGreaterThan(attachmentSlot);
    expect(workbench.indexOf('<DraftImagesPreview')).toBeGreaterThan(attachmentSlot);
  ...
  it('opens a full-width grouped add panel above the composer', () => {
    ...
    expect(sharedContextMenu).toContain('<DropdownMenuLabel>');
    expect(sharedContextMenu).toContain('<DropdownMenuSeparator />');
    expect(sharedContextMenu).toContain('side="top"');
    expect(sharedContextMenu).not.toContain('document.addEventListener');
    expect(sharedContextMenu).not.toContain('mf-workbench-context-separator');
  ...
  it('renders Skill execution as conversation content, never as a Composer trace', () => {
    ...
    expect(skillRuntimeConversation).toContain('data-testid="skill-runtime-conversation"');
    expect(skillRuntimeConversation).toContain('data-placement="conversation"');
    expect(skillRuntimeConversation).not.toContain('data-testid="skill-runtime-trace"');
