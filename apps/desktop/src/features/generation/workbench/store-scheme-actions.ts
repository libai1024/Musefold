...
import type {} from '@musefold/desktop-contracts/design-scheme';
...
export function createWorkbenchSchemeActions(
  set: WorkbenchSet,
  get: WorkbenchGet,
): WorkbenchSchemeActions {
  return {
      ...
      set((state) => ({
        turns: mapTurnsEverywhere(state.turns, (turn) => {
          ...
          const trace = turn.source.trace.some((existing) => existing.id === item.id)
            ? turn.source.trace.map((existing) =>
