...
import { filterCommandHints, parseDesignPlanBody, parseDesignPlanIntent } from './composerIntent';
...
export function useWorkbenchComposerStore() {
  ...
  const runningKindHere = useGenerationWorkbenchStore(
    (s) =>
      Object.values(s.runningTurns).find((entry) => entry.sessionId === s.sessionId)?.kind ?? null,
  );
  ...
  const designPlanIntent =
    draftCommand === 'design-plan' ? parseDesignPlanBody(prompt) : parseDesignPlanIntent(prompt);
  ...
  const skillComposerAttachmentVisible = ['detecting', 'ready', 'error'].includes(
    skillRuntimeStatus,
  );
  ...
  const schemeModifyCanSubmit = Boolean(
    !doubaoImageMode && schemeModifyMode && prompt.trim() && !generatingHere && !schemeCreating,
  );
  const canSubmit =
    ordinaryCanSubmit ||
    skillCanSubmit ||
