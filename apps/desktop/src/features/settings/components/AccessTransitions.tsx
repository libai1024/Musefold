...
export function AccountIdentityTransition({
  state,
  onSwap,
  onComplete,
}: {
    ...
    >
      <div ref={shellRef} className="w-[280px] max-w-[calc(100vw-40px)]">
        ...
        <div className="relative h-11 overflow-hidden">
          <div ref={fromRef} className="absolute inset-0 will-change-[transform,opacity]">
            <IdentityMark identity={state.from} />
          </div>
          <div ref={toRef} className="absolute inset-0 will-change-[transform,opacity]">
            <IdentityMark identity={state.to} />
          </div>
        </div>
        <div className="relative mt-4 h-px bg-border-default">
          <span
            ref={dotRef}
            className="absolute -top-[3px] left-0 h-[7px] w-[7px] rounded-full bg-accent will-change-transform"
          />
        </div>
      </div>
    </div>,
