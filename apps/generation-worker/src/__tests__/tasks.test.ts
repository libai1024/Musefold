...
import { decideLeaseRecovery, transitionMcpSpendReservation } from '../tasks.js';
...
describe('Cloud MCP spend reservation lifecycle', () => {
  it('settles a successful generation at the reserved estimate', async () => {
    ...
    await transitionMcpSpendReservation({ query } as never, 42, 'run-success', 'settled');

    expect(query).toHaveBeenCalledOnce();
    expect(query.mock.calls[0]?.[0]).toContain('actual_points = estimated_points');
  ...
  it('releases an unspent reservation after a definitive failure', async () => {
    ...
    await transitionMcpSpendReservation({ query } as never, 42, 'run-rejected', 'released');
  ...
  it('preserves a reservation when the upstream charge is uncertain', async () => {
    ...
    await transitionMcpSpendReservation({ query } as never, 42, 'run-unknown', 'preserved');
