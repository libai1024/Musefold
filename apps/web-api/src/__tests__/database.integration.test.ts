...
import { DatabaseRuntime, GenerationEventNotifier } from '../database/runtime.js';
...
databaseTests('PostgreSQL foundation', () => {
  ...
  beforeAll(async () => {
    ...
    `);
    await runMigrations({
      connectionString: withCredentials(adminUrl, 'musefold_worker', 'musefold_worker'),
    });
    await runner({
  ...
  it('keeps owner records isolated by request-scoped database context', async () => {
    ...
    const unscoped = await appPool.query('SELECT count(*)::int AS count FROM app.cloud_accounts');
  ...
  it('creates the expected schemas, extension and migration ledger', async () => {
    ...
    `);
    expect(result.rows.map((row) => row.schema_name)).toEqual(['app', 'auth', 'ops']);
  ...
  it('enforces rate limits atomically across concurrent API requests', async () => {
    ...
    try {
      const limiter = new PostgresRateLimiter(runtime.db, 'integration-rate-limit-secret');
  ...
  it('stores only encrypted opaque Web sessions through narrow auth functions', async () => {
    ...
    try {
      ...
      `);
      expect(persisted.rows[0]?.plaintext).not.toContain('sk-generation-secret');
    } finally {
      await runtime.close();
    }
  });
  ...
  it('supports device registration, idempotent push, pull and bootstrap', async () => {
    ...
    try {
      ...
      expect(concurrentResults.map((result) => result.results[0]?.status).sort()).toEqual([
        'applied',
        'duplicate',
      ]);
      ...
      await expect(prompts.getPrompt(202, relatedPromptId)).resolves.toMatchObject({
        folderId: null,
        tags: [],
        version: 3,
      });
      await expect(prompts.getFolder(202, childFolderId)).resolves.toMatchObject({
        parentId: null,
        version: 2,
      });
      ...
      await expect(sync.pushUsage(202, deviceId, [usageEvent])).resolves.toEqual({
        results: [{ eventId: usageEvent.eventId, status: 'applied', errorCode: null }],
      });
      await expect(sync.pushUsage(202, deviceId, [usageEvent])).resolves.toEqual({
        results: [{ eventId: usageEvent.eventId, status: 'duplicate', errorCode: null }],
      });
  ...
  it('creates one queued generation per idempotency key and enqueues the fixed task', async () => {
    ...
    try {
      ...
      const generations = new GenerationService(runtime.db, signer, 'integration-approval-secret');
      const input = {
        prompt: '一张极简产品海报',
      ...
      const first = await generations.create(101, input, 'generation-idempotency-0001');
      const repeated = await generations.create(101, input, 'generation-idempotency-0001');
      ...
      await expect(generations.assetSignedUrl(202, assetId)).rejects.toMatchObject({
  ...
  it('serializes Cloud MCP daily budgets and preserves approval retries', async () => {
    ...
    try {
      ...
      expect(concurrent.filter((result) => result.job.status === 'queued')).toHaveLength(3);
      expect(concurrent.filter((result) => result.job.status === 'pending_approval')).toHaveLength(
        7,
      );
      ...
      const queued = concurrent.find((result) => result.job.status === 'queued');
      if (!queued) throw new Error('Expected one queued budget result');
      await generations.cancel(101, queued.job.id);
      const replacement = await generations.createCloudMcp(101, input, 'budget-after-release', {
        grantId: autoGrantId,
        approvalRequired: false,
      });
      ...
      expect(tokenRow.rows[0]?.approval_token_hash).not.toBe(firstApproval.approvalToken);
      ...
      await expect(
        generations.approveCloud(101, firstApproval.job.id, firstApproval.approvalToken ?? ''),
      ).resolves.toMatchObject({
        status: 'queued',
      ...
      expect(connections.find((connection) => connection.id === autoGrantId)).toMatchObject({
      ...
      await expect(
        oauth.updateConnection(101, autoGrantId, { maxPointsPerDay: 4_000 }, true),
  ...
  it('grants the read-only MCP scopes with the default budget and supports scope edits', async () => {
    ...
    try {
      await adminPool.query(
        `INSERT INTO auth.oauth_clients(client_id, client_name, redirect_uris)
         VALUES ('default-open-client', 'Default Open', '[]'::jsonb)
         ON CONFLICT (client_id) DO NOTHING`,
      );
      const oauth = new OAuthService(runtime.db);
      await expect(
      ...
      const grant = await oauth.ensureGrant(101, 'default-open-client', ['account:read']);
      ...
      await expect(
        oauth.updateConnection(101, grant.id, { scopes: ['account:read', 'prompts:read'] }, true),
      ).resolves.toBeUndefined();
      // 收窄能力无需重认证。
      await expect(
      ...
      const connections = await oauth.listConnections(101);
      expect(connections.find((connection) => connection.id === grant.id)?.scopes).toEqual([
        'account:read',
        'prompts:read',
        'skills:read',
      ]);
      ...
      const afterReconsent = (await oauth.listConnections(101)).find(
        (connection) => connection.id === grant.id,
      );
...
function withCredentials(connectionUri: string, username: string, password: string): string {
