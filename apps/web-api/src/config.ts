...
export const webApiConfigSchema = z.object({
  ...
  MCP_RESOURCE_URL: z.string().url().default('http://127.0.0.1:60160/api/musefold/mcp'),
  NEW_API_BASE_URL: z.string().url().default('http://127.0.0.1:3000'),
  SESSION_COOKIE_NAME: z
    .string()
    .regex(/^[A-Za-z][A-Za-z0-9_]{0,63}$/)
    .default('mf_session'),
  SESSION_ENCRYPTION_KEY: z.string().min(16).default('development-only-musefold-session-key'),
  OAUTH_JWKS_JSON: z.string().trim().min(1).optional(),
  SESSION_IDLE_TTL_SECONDS: z.coerce.number().int().min(300).max(2_592_000).default(604_800),
  ...
  S3_SECRET_ACCESS_KEY: z.string().min(1).max(256).default('musefold_local_secret'),
...
export function loadWebApiConfig(environment: NodeJS.ProcessEnv = process.env): WebApiConfig {
  ...
  if (
    ...
    throw new Error('SESSION_ENCRYPTION_KEY must be configured outside development');
  }
