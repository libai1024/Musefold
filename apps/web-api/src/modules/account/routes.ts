...
interface AccountRoutesOptions {
  ...
  config: Pick<WebApiConfig, 'NODE_ENV' | 'SESSION_COOKIE_NAME' | 'SESSION_ABSOLUTE_TTL_SECONDS'>;
...
export const accountRoutes: FastifyPluginAsync<AccountRoutesOptions> = async (app, options) => {
  ...
  app.post(
    '/api/musefold/v1/auth/device-session',
    {
      schema: {
        tags: ['account'],
        headers: z.object({ authorization: z.string().min(16).max(8_192) }).passthrough(),
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      reply.setCookie(options.config.SESSION_COOKIE_NAME, result.rawSessionId, cookieOptions);
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const result = await options.accountService.login(loginRequestSchema.parse(request.body));
      reply.setCookie(options.config.SESSION_COOKIE_NAME, result.rawSessionId, cookieOptions);
      return publicSession(result);
    },
  );
  ...
  app.post(
    '/api/musefold/v1/auth/redeem',
    {
      schema: {
        tags: ['account'],
        headers: z.object({ 'x-musefold-csrf': z.string().min(32).max(256) }).passthrough(),
        body: redeemRequestSchema,
        response: { 200: redeemResultSchema },
      },
    },
    async (request) => {
      const rawSessionId = requireCookie(request.cookies, options.config.SESSION_COOKIE_NAME);
...
function publicSession(result: AccountSession & { rawSessionId: string }): AccountSession {
  return { account: result.account, csrfToken: result.csrfToken };
}

function requireCookie(cookies: Record<string, string | undefined>, name: string): string {
  const value = cookies[name];
  if (!value) throw new AppError('AUTH_REQUIRED', '请先登录 Musefold', 401);
  return value;
}

function requireCsrf(header: string | string[] | undefined, expected: string): void {
