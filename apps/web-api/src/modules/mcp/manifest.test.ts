...
import { CLOUD_MCP_TOOL_MANIFEST, CLOUD_MCP_TOOL_NAMES, enabledCloudMcpTools } from './manifest.js';
...
describe('Cloud MCP tool manifest', () => {
  it('is the exact seven-tool read-only allowlist', () => {
    ...
    expect(CLOUD_MCP_TOOL_MANIFEST.some((entry) => forbiddenNames.includes(entry.name))).toBe(
      false,
    );
  });
