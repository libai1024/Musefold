...
export const mcpRoutes: FastifyPluginAsync<McpRoutesOptions> = async (app, options) => {
  app.post('/api/musefold/mcp', { bodyLimit: 512 * 1024 }, async (request, reply) => {
    const origin = request.headers.origin;
    if (typeof origin === 'string' && origin !== new URL(options.config.PUBLIC_ORIGIN).origin) {
...
async function authenticate(
  ...
  try {
    return await oauth.verifyAccessToken(authorization.slice(7).trim(), resource);
  } catch (error) {
    const appError =
