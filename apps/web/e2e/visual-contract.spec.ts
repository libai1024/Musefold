...
async function captureCanonicalSurface(
  ...
  if (testId === 'generation-result-group') {
    ...
    for (let index = 0; index < count; index += 1) {
      await expect(results.nth(index)).toHaveAttribute('data-theater-idle', 'true');
    }
  }
  ...
  if (outputDir) {
    ...
    const sharedFileName: string | undefined = (
      {
        ...
        'product-sidebar-1440-light-comfortable': 'shared-product-sidebar-1440x900.png',
        'workbench-composer-1440-light-comfortable': 'shared-workbench-composer-1440x900.png',
        'workbench-composer-390-light-comfortable': 'shared-workbench-composer-390x844.png',
        'workbench-result-1440-light-comfortable': 'shared-workbench-result-1440x900.png',
        ...
        'library-list-1440-light-comfortable': 'shared-library-list-1440x900.png',
        'prompt-detail-1440-light-comfortable': 'shared-prompt-detail-1440x900.png',
        'prompt-reference-card-1440-light-comfortable': 'shared-prompt-reference-card-1440x900.png',
        'prompt-reference-preview-1440-light-comfortable':
          'shared-prompt-reference-preview-1440x900.png',
        'history-workspace-1440-light-comfortable': 'shared-history-workspace-1440x900.png',
    ...
    if (name === 'account-1440-light-comfortable') {
      ...
      await accountSummary.locator('.mf-account-summary-header-action').evaluate((element) => {
...
test('canonical Desktop/Web surfaces stay within the shared visual contract', async ({
  ...
  await captureCanonicalSurface(page, testInfo, 'account-1440-light-comfortable', 'account-screen');
  await page
...
test('generation result failure and cancellation states stay shared across viewports', async ({
  ...
  await page.getByLabel('生成图片').click();
  await expect(page.locator('.mf-generation-result-surface[data-status="failed"]')).toBeVisible({
    timeout: 8_000,
  });
  await captureCanonicalSurface(
    page,
  ...
  await expect(page.locator('.mf-generation-result-surface[data-status="cancelled"]')).toBeVisible({
    timeout: 8_000,
  });
  await captureCanonicalSurface(
    page,
