...
test("desktop prompt to generation to history flow", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.getByTestId("generation-user-message-actions"),
  ).toBeVisible();
  ...
  await expect(page.getByTestId("generation-save-prompt")).toHaveText(
    "已存为提示词",
  );
  ...
  const lifecycleRow = page.locator(
    '[data-prompt-id="prompt-night-architecture"]',
  );
  ...
  const historyGeometry = await page.evaluate(() => {
    ...
    const list = workspace?.querySelector<HTMLElement>(
      ".mf-history-workspace-list",
    );
  ...
  expect(historyGeometry?.list ?? 0).toBeGreaterThan(
    historyGeometry?.inspector ?? 0,
  );
  expect(
    (historyGeometry?.list ?? 0) + (historyGeometry?.inspector ?? 0),
  ).toBeCloseTo(historyGeometry?.workspace ?? 0, 0);
  await expect(page.getByTestId("history-detail-prompt")).toContainText(
    "雨后的夜间建筑摄影",
  );
...
test("shared empty state and composer popovers match desktop interactions", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  const browserErrors = collectBrowserErrors(page);
  ...
  expect(ratioBox!.y + ratioBox!.height).toBeLessThanOrEqual(
    composerBox!.y + 1,
  );
  await page.getByTestId("refine-ratio-16:9").click();
  await expect(page.getByTestId("refine-ratio-trigger")).toHaveAttribute(
    "data-value",
    "16:9",
  );
  ...
  const settingsLabel = page
    .getByTestId("workbench-more-settings")
    .locator("span");
  ...
  await expect(page.getByTestId("workbench-more-settings")).toContainText(
    "超清",
  );
...
test("shared composer auto-resizes and keeps Desktop keyboard semantics", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await waitForFixtureWorkspace(page);
...
test("shared prompt editor keeps discard, collapse and shortcut interactions", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  const browserErrors = collectBrowserErrors(page);
...
test("mobile generation flow remains usable without overflow", async ({
  page,
}, testInfo) => {
...
test("cancelled workbench results can be retried in place", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  const browserErrors = collectBrowserErrors(page);
  ...
  await expect(
    resultSurface.getByRole("button", { name: "重试" }),
  ).toBeVisible();
  ...
  await expect(
    page.locator('.mf-generation-result-surface[data-status="success"]'),
  ).toBeVisible();
  await expectNoHorizontalOverflow(page);
  expect(browserErrors).toEqual([]);
});

test("recent conversations restore and archive from the shared sidebar", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  const browserErrors = collectBrowserErrors(page);
  ...
  await sessionRow
    .getByRole("button", { name: promptText, exact: true })
    .click();
  ...
  await sessionRow
    .getByRole("button", { name: `归档聊天：${promptText}` })
    .click();
...
test("shared account and Cloud MCP connection policies keep their actions deterministic", async ({
  ...
  await expect(page.getByTestId("account-summary-panel")).toContainText(
    "186 积分",
  );
  await page.getByRole("button", { name: "退出登录" }).click();
  await expect(
    page.getByRole("heading", { name: "登录个人账户" }),
  ).toBeVisible();
  ...
  await expect(
    connection.getByRole("button", { name: "恢复连接" }),
  ).toBeVisible();
...
test("recent conversation actions stay shared with Desktop", async ({
  page,
}) => {
  await page.setViewportSize({ width: 1280, height: 720 });
  await waitForFixtureWorkspace(page);
  ...
  await expect(row.getByTestId("conversation-status-dot")).toHaveAttribute(
    "data-status",
    "unread",
  );
...
test("shared sidebar resizes, collapses, and becomes a compact drawer", async ({
  page,
}) => {
