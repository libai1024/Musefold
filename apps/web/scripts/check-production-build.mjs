...
const javascript = (
  await Promise.all(files.map((file) => readFile(join(assetsDirectory, file), 'utf8')))
).join('\n');
...
if (leaked.length > 0) {
  throw new Error(`Production Web bundle contains fixture markers: ${leaked.join(', ')}`);
}

console.log(
  `Production boundary check passed (${files.length} JavaScript asset${files.length === 1 ? '' : 's'}).`,
);
