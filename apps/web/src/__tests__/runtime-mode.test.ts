...
describe('resolveWebGatewayMode', () => {
  ...
  it('requires an explicit true flag for fixture previews', () => {
    expect(resolveWebGatewayMode({ isDevelopment: true, useFixtures: 'true' })).toBe('fixture');
    expect(resolveWebGatewayMode({ isDevelopment: true, useFixtures: 'false' })).toBe('api');
  });

  it('never enables fixtures in production', () => {
    expect(resolveWebGatewayMode({ isDevelopment: false, useFixtures: 'true' })).toBe('api');
  });
});
