...
**实现摘要**：
- `resetBusinessData()` 先创建 `pre-reset` 一致性快照，再在单事务内按外键逆序清空业务表与 `prompts_fts`（v2.5 起 `RESET_ORDER` 含 `workbench_drafts` / `workbench_sessions`，共十表；此前只清七类业务表会在工作台留下无轮次的空对话）。
...
- [x] 需输入匹配短语才启用「永久清空」
- [x] 执行前自动备份，返回并提示备份路径
- [x] 清空后业务数据（含 v2.5 工作台会话/草稿）为空、FTS 同步清空、历史清空
