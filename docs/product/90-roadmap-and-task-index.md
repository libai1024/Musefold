...
- 打包 manifest 防裁剪（2026-08-06）：`package:*` 脚本改经 `scripts/run-builder.mjs` 调用 electron-builder，并在打包结束后自动恢复根 `package.json`；本机重跑 macOS 与 Windows ARM64 打包后均验证 scripts/devDependencies 未被运行时 metadata 裁剪。
