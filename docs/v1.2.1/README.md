...
**当前状态（2026-08-20）**：仓库侧 M1/M4/M5/M7 已完成；生产部署、对象存储/CDN、真实热更新、签名公证与非 root 部署身份等发布门禁仍未完成，因此 v1.2.1 尚未达到发布完成状态。
...
- `.github/workflows/ci.yml` 与 `package-smoke.yml` 已提供验证和 tag 打包冒烟；真实签名、公证、CDN 上传及生产自动部署尚未跑通。
- `V121-CHAN-07` 的对象存储/CDN、受限部署身份、真实 dev 通道热更新和生产回滚仍是外部门禁。
