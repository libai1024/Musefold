...
**当前进度（2026-08-21）**：Phase 0、Phase 1a/1b、Phase 2（含 WebGateway 显式端口绑定、settings 补卡与全 renderer 宿主 IO 收口）及 Phase 3 均已完成，v1.2.2 仓库开发完成。全仓 Turbo 28/28（Phase 1b 下移 App manifest 后的最终任务图）、根测试 180 files / 1039 tests、桌面 E2E 222 passed / 17 skipped、Web E2E 19 passed、共享视觉门禁、macOS ad-hoc 真包运行态、Windows ARM64 结构包、Web API 精简 Docker 镜像和发布预检均通过。Developer ID/公证、远端 CI、Windows 目标设备与生产发布仍是 v1.2.1 外部 evidence 门禁，不属于「本机已验证」的宣称。
...
2. **桌面原本没有数据访问抽象**。Web 有 `WebGateway`，桌面曾有 47 个渲染文件直接 import IPC，18 个 zustand store 就是数据层。Phase 2 已上提六端口并落地 `DesktopGateway`、library 写路径样板，以及 `DesktopExtras`（library/account/cloudSync、provider/aiConnection、workbench 无损会话/原生进度面）；工作台 store 已用可注入 `WorkbenchIO` 与三 controller 拆开 transport 和状态机。桌面独有的 system/pet/automation/designScheme 等壳能力统一经 `DesktopHostServices`，renderer 业务文件不再直接 import `lib/ipc`；裸 `window.api` 只剩 5 个低层桥接入口。桌面已接入 `getProductCapabilities('desktop')`（见[架构文档第 4 节](./V122-ARCHITECTURE.md)）。
3. **桌面 App 曾占据仓库根目录**（Phase 1a 前：`src/` + `electron/` + `shared/` 约 6.5 万行），导致三套互不一致的路径别名、无 project references、typecheck 入口分裂为三条。Phase 1a 已迁走源码并解散 `shared/`；Phase 1b 已将 App manifest、运行依赖与 builder 配置下移，根 manifest 只负责 workspace 工具链。
...
| Phase 1b App manifest 下移 | 根 package.json 变纯 workspace root；打包与发布路径同步 | **已完成（2026-08-20）**：DIR-06~09，双平台结构包与 Docker 实证通过 |
