...
**当前进度（2026-08-20）**：Phase 0 工程化地基与 Phase 1a 源码目录迁移已完成；Phase 2 桌面 Gateway 部分完成（GW-01 domain / GW-02 / GW-03 / GW-05 / GW-07（两刀 fa45f74 + 83d9f71）/ GW-08 已完成；GW-01 的 WebGateway `implements` 补卡等待 web 并行工作流收口；GW-04 / GW-06 未开工）；Phase 3 共享逻辑归位部分完成（SHARE-06 / 01 / 05 / 02 / 03 已完成；仅剩 SHARE-04，与 Phase 2 stores 切换同批）；Phase 1b（App manifest 下移）未开工。
...
2. **桌面没有数据访问抽象**。Web 有 `WebGateway`，桌面 47 个渲染文件直接 import IPC，18 个 zustand store 就是数据层。Phase 2 已上提六端口并落地 `DesktopGateway` 骨架、library 写路径样板，以及 `DesktopExtras`（GW-07 两刀 fa45f74 + 83d9f71：library 查询/写面走行模型；account login/status 全量与 cloudSync 直通 AccountStatus / CloudSyncSummary）。settings 的 aiConnection / provider 仍直连 ipc，未纳入 GW-07。桌面已接入 `getProductCapabilities('desktop')`（见[架构文档第 4 节](./V122-ARCHITECTURE.md)）。
...
| Phase 2 桌面 Gateway | domain 端口做全、`DesktopGateway`、stores 逐个切换 | **部分完成（2026-08-20）**：GW-01（domain）/ 02 / 03 / 05 / 07（两刀 fa45f74 + 83d9f71）/ 08 已完成；GW-01 WebGateway `implements` 补卡等待 web 并行工作流收口；GW-04 / 06 未开工 |
