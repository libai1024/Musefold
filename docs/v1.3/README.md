...
**当前进度（2026-08-21）**：文档冻结，实施未开始。前置版本 [v1.2.2 系统架构重构](../v1.2.2/README.md) 已完成（Phase 0–3 全部落地，depcruise 0 违规），其迁移计划第 6 节预埋的「实体统一」「宿主编排收敛」两个 v1.3+ 候选由本版本接棒。
...
| 阶段 | 内容 | 依据 |
|---|---|---|
| Phase 0 治理地基 | GOV-01~04：尺寸棘轮、feature 隔离、命名统一、ipc/preload 分域 | 规则先行（v1.2.2 交付原则 3），防拆分后回潮 |
| Phase 1 实体统一 | ENT-01~04：行模型 storage-only 止血 → `DesktopExtras` 逐域文档化 → stores 类型切换 → `models.ts` 收缩 | 编排层操作统一实体才有意义，先于 Phase 2 |
| Phase 2 状态与编排 | STATE-01~03 + ORCH-01~04：Query 引入与读路径迁移；编排 hook 下沉、`App.tsx` 拆解、桌面切换同一编排层 | 共享编排需要平台无关取数（Query 先行） |
