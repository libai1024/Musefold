...
最大的一条：**图库与历史页仍是双端各写一套**——工作台已经双端共享，这两个页面没跟上，ORCH 卡下沉的是 controller，视图组合没下沉。其余包括 ENT-B（SQLite schema 迁移）、RHF+Zod 零采用（规范与现实不符）、Web 手写 CSS 与桌面 Tailwind 未统一、`max-lines` 尾部 12 条。完整清单与各自的触发条件见[迁移计划 §8](./V13-MIGRATION-PLAN.md)。
