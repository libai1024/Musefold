...
顶部控制区进一步参考 [ZCode 插件页](./references/zcode/zcode-plugins-dark.png)：范围标签和紧凑搜索同排，结果摘要和操作另起一排。

## 3. 方案列表布局

```text
┌──────────────────────────────────────────────────────────────┐
│ [我的方案 12] [发现]             [搜索方案、作者或仓库]     │
│ 我的方案 12                              [刷新] [新建]      │
├──────────────────────────────────────────────────────────────┤
│ 我的方案                                              12    │
│ ┌────┐  雨天视觉方向             [完整还原] [使用]      │
...
- 第一行左侧是 `我的方案 / 发现` 范围标签，右侧是紧凑搜索框。
- 第二行左侧是当前范围名称与结果数量，右侧是刷新和新建。
- 方案列表和右侧 Inspector 不因控制区升级改变宽度或叠加关系。
- `发现` 的远程提交使用搜索框尾部图标按钮，同时支持 Enter；不再增加独立文字按钮。

规格：

- 页面标题仍由桌面 TitleBar 承载，不在内容区重复。
- 标签高度 32px，圆角 8px；选中态为 `--bg-active`、极细内描边和 `--shadow-sm`。
- 搜索框高度 36px、圆角 8px、桌面宽度不超过 400px。
- 搜索图标 14px；清空和提交均为 26x26px 图标按钮。
- 刷新为 34x34px outline icon button，新建高度 34px、圆角 8px。
- 新建菜单仍挂载在新建按钮下方，不随搜索框位置改变。
...
## 30. Phase C 方案页浮层实现（2026-08-26）

本批针对方案页做逐实例浮层升级，继续保持 ZCode 的“低频动作收进更多、当前动作留在工作区”原则。

### 30.1 方案详情操作菜单

详情页标题区的 `...` 是唯一低频动作入口，使用共享 `DropdownMenu`，不再由方案组件自行监听 `pointerdown` 或绘制绝对定位菜单。
...
- 正式方案显示“在 Composer 中修改、查看来源、检查更新、导出分享包”；草稿显示“重命名、查看来源”，GitHub 来源按条件显示“检查更新”。
- 危险动作固定放在真实 separator 后，使用 danger text 和 danger hover，不与普通动作混淆。
- 菜单宽度 200px，右边缘与 `...` 对齐，外圆角由共享 Popover token 控制为 8px，阴影使用 `--shadow-pop`。
- 打开后首个可用 menu item 获得焦点；方向键、Home / End、Escape 和触发器焦点归还由共享原语提供。

### 30.2 新建来源菜单

顶部“新建”仍是带文字的明确命令，但来源菜单改为共享 Dropdown Portal，以保证菜单不会被列表滚动容器裁剪。

- 菜单宽度 286px，外圆角 8px，内部使用 6px 行圆角和 32px 基础行高。
- 五个来源保留两行信息：16px 图标、来源标题、11px 说明；hover 只使用 soft surface。
- 菜单打开时不改变列表、Inspector 或主工作区宽度；Escape 关闭并归还“新建”按钮焦点。

### 30.3 运行 Composer 可选变量

必填变量直接显示在 Composer 的变量区；可选变量仍通过“+ 变量”按需加入，但选项菜单使用共享 Dropdown。
...
- 菜单从变量按钮上方展开，宽度 220px，避免遮住底部提交按钮。
- 选择变量后菜单自动关闭，字段进入稳定的变量网格；切换 revision 时清空已手动添加的可选字段。
- 变量菜单只负责选择，不承担变量值校验；必填状态仍由字段中的文字、输入状态和 Ember 小标记共同表达。
- 方案附件详情继续使用 dialog 语义的富信息浮层，因为它包含方案摘要、来源、输入要求和“查看详情/更换/移除”动作，不应退化成普通菜单。

### 30.4 方案页桌面验收

- [x] 详情 `...`、新建来源、可选变量三类菜单使用共享 Dropdown 原语。
- [x] Light / Dark 使用实色 popover surface、统一边框与 `shadow-pop`。
- [x] 菜单不改变三栏工作区几何，不被父级 overflow 裁剪。
- [x] 首项焦点、Home / End、Escape 和触发器焦点归还由共享原语覆盖。
- [x] 删除草稿 / 移除方案仍进入独立 Dialog，保留历史图片的业务语义。
- [x] 本批仅执行桌面端验证，不执行手机端测试。
