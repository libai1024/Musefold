...
**2026-09-23 状态：** v2.5 功能迁移基本完成（任务包 93/98）；「同一源码上的完整验收、生产对齐与发布」的欠账已由 [v2.5.1 收口审计](../v2.5.1/README.md) 统一清点，修复按其[执行计划](../v2.5.1/EXECUTION-PLAN.md)与[工作表](../v2.5.1/REPAIR-PLAN.md)推进。09-21 真实验收、09-22 独立云端部署与 CLI/MCP 修复见索引新增各行；Windows、签名公证、远端 CI 与正式发布仍未完成。

**2026-09-20 收口快照：** 账号模式的云端模型选择、价格展示及提交前权威校验已接入普通生成与方案运行；最终全量回归、当前产物和原定外部条件仍在验收，尚未完成迁移或提交发布。当前任务与报告以[全迁移收口记录](./V25-CLOSEOUT-2026-09-19.md)和[任务清单](../../.spec/specs/2026-09-01_migrate-v21-to-v25/tasks.md)为准。下方带日期的批次结果只证明当时源码与范围，不作为当前通过结论。
...
| 文档 | 内容 |
|---|---|
| [V25-REAL-ACCEPTANCE-2026-09-21.md](./V25-REAL-ACCEPTANCE-2026-09-21.md) | 09-21 真实跨端与收费验收：跨端同步、方案各节点逐段证据、多轮门禁结果与当日费用（2026-09-23 已补登 16:02 后终态） |
| [V25-NONWINDOWS-ACCEPTANCE-2026-09-20.md](./V25-NONWINDOWS-ACCEPTANCE-2026-09-20.md) | 09-20 非 Windows 验收：登录会话机制本地全量、上游账号扩展生产部署与 1850 项 `bb580e98…` 全绿基线 |
| [V25-CLOUD-DEPLOYMENT.md](./V25-CLOUD-DEPLOYMENT.md) | 09-22 独立云端部署：`/Musefold/v25` 命名空间隔离、独立库/对象/角色、公网真实登录与旧版保留 |
| [V25-CLI-MCP-CLOUD-FIX.md](./V25-CLI-MCP-CLOUD-FIX.md) | 09-22 CLI/MCP 普通云账号生图修复：真实安装包验收、三张实图费用分层与提交绑定（`2561d2a`） |
| [V25-MIGRATION-TESTING-CONTINUED.md](./V25-MIGRATION-TESTING-CONTINUED.md) | B63+ 后续分批结果与限制：资产 GC 阶段 4、D02PURGE、云端 G 参考图与 G-RELEASE-01 基础 |
| [V25-LOGIN-SESSIONS.md](./V25-LOGIN-SESSIONS.md) | 09-20目标账号SSH释放实证；自动回收、登录设备管理与满额登录机制已实现并部署上游账号扩展（v1.1 公共站点切换另行授权） |
