...
### P0 — 存量用户数据目录冻住主进程 Node 定时器

- **2026-09-25 晚间复测未复现**:同一台机器、同一真实 userData,冷启动官方 2.5.0 后 automation
  探测连续 6 次(约 50 秒窗口)全部亚秒响应(0.1–0.6s),主进程定时器正常泵送;当日全天生图/轮询
  也均正常。复现条件(锁屏?特定数据状态?时段?)待定,冻结根因仍未定位,问题保持开放。
- **现象**:应用以用户日常 userData(`~/Library/Application Support/musefold-app`)启动后,主进程的 JS 事件循环停止泵定时器:
