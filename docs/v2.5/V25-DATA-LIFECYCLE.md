...
**2026-09-23 状态：** 本表基线已推进至 09-21：阶段 4 各项（上传 24h TTL/迟到 PUT/慢复制/导入孤儿启动 GC/方案硬删除 GC 出口/D02.7 安全快照）与 D02PURGE 已实现并按各自范围验证，见测试续篇 §5.101–§5.104 与 §5.113；任务包当前 93/98。数据线余项（调度排程、期限上界、双进程矩阵、桌面自动保留政策等）以 [v2.5.1 审计](../v2.5.1/README.md) §5.3 为准。以下 09-14 段落保留为历史。
...
#### 18.6.1 长驻Automation上传接续验收边界（已实现,2026-09-23 标注:上传 24h TTL/自动释放、迟到 PUT、崩溃孤儿启动 GC 等已由测试续篇 §5.101/§5.102 实现并验证;自然期限实测与双进程矩阵仍开放,见 v2.5.1 §5.3）
