...
- **当前证据边界**:当前分支 `spec/2026-09-01_migrate-v21-to-v25`；阶段成果已含 checkpoint `607e7b5`。当前树最终复现 `pnpm run check`（180 个测试文件、1301 个测试、35 个 Turbo task，退出码 0）；本轮 Design Scheme 受影响组合为 8 个文件、151 个测试，根 `tsc -b` 通过，独立复审无剩余 finding。History 成本、Workbench 贴底、正式方案修改入口、相册键盘/触控与移动 Workbench 视觉等既有证据保留；最新完整 `pnpm run test:e2e` 仍为 97 passed、1 failed、5 skipped，唯一失败是 macOS runner 原生 fullscreen 前台焦点阻塞。由于没有 durable machine-readable report/artifact，本地数字不把 B00 升为 `done`。
- **动作**:
...
- **走线**:contracts、platform seam、Desktop adapter/SQLite/runtime、Web cloud CRUD client 和 shared features 已建立。Desktop canonical run/cancel/event transport 已接通，本轮再以 text-only `prepareRun` 由主进程生成权威固定计划；剩余为 Desktop Workbench `onSubmit`、图片 prepare、Agent create/modify/recompile、Web run/assets/package 与三形态 E2E。
...
- **宿主模型裁决**:
  ...
  - capability 必须按宿主真实状态表达：Desktop `hasDesignSchemes=true`，Web `hasDesignSchemes=false`。Desktop 入口可展示已接通的列表/详情/确定性动作，Workbench 运行提交仍因缺少 `onSubmit` 禁用并解释；Web 不显示入口。不得用 `isElectron`、UA、路径字符串或空实现冒充能力。
...
- **Difference**:`N/A`(本卡只扩展 Desktop transport 与主进程权威运行准备；不改变共享 UI 产品动作，Workbench 提交仍关闭)。
...
| P01-10 完整生命周期编排 | GPT | GLM | P01-4,P01-5,P01-6,P01-9 | todo | todo | doing | partial | deterministic lifecycle、working-draft promotion、canonical run/cancel/event、`607e7b5` cancellation-wins 与主进程权威 text-only `prepareRun` 已完成；Desktop Workbench 提交、图片 prepare、Agent create/modify/recompile、Web runtime 和跨宿主成功证据尚未完成 |
...
|---|---|---|---|---|---|---|---|---|
| B00 | 主代理 | — | verify | verify | verify | N/A | verify | 当前树 `pnpm run check` 已通过（180 个测试文件、1301 个测试、35 个 Turbo task）；最新完整 v25 E2E 为 97 passed、1 failed、5 skipped，唯一失败是原生 macOS fullscreen runner 前台焦点阻断；本轮 Electron project 单独复跑为 32 passed、1 skipped、1 failed，失败原因相同；无 durable report/artifact 绑定，仍不能登记为 done/pass |
...
| P01-0..P01-13 设计方案 | GPT/Kimi | GLM | doing | doing | doing | partial | doing | contracts、SQLite/PG、Desktop/API/client adapters、shared features、working draft、Desktop 导入导出及导航/详情已有证据；Desktop capability true，canonical run/cancel/event 与 text-only `prepareRun` 已接线，受影响组合 151 tests 通过；Desktop Workbench 提交、图片 prepare、Agent create/modify/recompile、真实 E2E 及 Web runtime/assets/package 尚未闭环，Web capability false |
