...
| 设计方案全生命周期 | baseline `features/design-schemes/*` 与 main runtime | canonical contracts、SQLite/PG schema、Desktop/API/client adapter、shared features | `partial`;确定性 CRUD、working-draft 读取与 Desktop `.musefold.design` 安全 staging/archive/domain 导入导出已接线（97 项聚焦 lifecycle/transport 用例 + 定向 typecheck/build，见 P01-4 登记），但 Desktop Agent 编译/modify/run/event 与真实 E2E、Web run/assets/package 尚未闭环，双宿主 capability 保持关闭 | P01-1..P01-13 |
...
- **当前证据边界**:当前分支 `spec/2026-09-01_migrate-v21-to-v25`;当前工作树含大量 `working-tree-only` 改动。已在本轮当前树复现 `pnpm run check`（179 个测试文件、1277 个测试、35 个 Turbo task，退出码 0），并完成定向 Web/Electron 回归；完整 v25 E2E 的原生 macOS fullscreen 用例仍受 runner 无法让 Electron BrowserWindow 获得前台焦点阻断，另有视觉基线差异已分别处理。由于本轮未提交且没有绑定 commit 的 machine-readable report/artifact，数字只能作为当前工作树验证记录，不能把 B00 升为 `done`。同步四态截图若位于 gitignored `.results`，仍不能替代持久 visual baseline。
- **动作**:
...
- **状态**:`doing`
- **平台矩阵**:域已从规划转入实施——Mobile Web:`doing`(共享 features 已有响应式形态,`/design-schemes` 入口/路由未挂载);PC Web:`doing`(同一 features 的桌面视图与 Inspector 已有,入口未挂载);Desktop:`doing`(v25 IPC/SQLite 确定性 CRUD 与 `.musefold.design` 导入导出已接线,导航入口、run/Agent 编译/modify/event 与 capability 未完成;macOS/Windows/Linux 差异按 capability 记录);Desktop Agent:`partial`(现有 Automation/CLI/local MCP 正式方案 list/get/compile/run 保留,不替代 Desktop renderer 证据)。
...
- **旧版/存量事实**:
  ...
  - 当前 v2.5 已有 shared contracts、PG/SQLite scaffold、platform seam、Desktop 确定性成功 adapter、Web API/client 与 `packages/features` 共享屏,但宿主入口仍未挂载、双宿主 capability 关闭。旧 renderer/legacy runtime 不能证明新壳可达。Mobile Web、PC Web、Desktop renderer 的入口均为 `todo`(实现层进度见 P01-4..P01-7);Desktop Agent 仅因 Automation/CLI/local MCP 存量兼容面记为 `partial`。
- **宿主模型裁决**:
...
| P01-4 Desktop v25 IPC | GPT | GLM | P01-1,P01-3 | N/A | N/A | doing | partial | 16 项部署方法的 transport、确定性 SQLite CRUD 与 `.musefold.design` staging/archive/domain 导入导出已接线并有定向测试(97 项聚焦 lifecycle/transport 用例 + typecheck/build);Agent create/modify、run/cancel、recompile、事件 producer、媒体与真实 E2E 仍是 blocker，capability 关闭 |
...
| P01-6 共享 schemes features | Kimi | GLM | P01-3,P01-4,P01-5 | doing | doing | doing | N/A | `packages/features/src/design-schemes` 已含列表/详情/来源/相册/版本/跨屏 integration；formal=current、trial/modify=exact working draft 已有回归测试，宿主闭环与 E2E 未完成 |
| P01-7 三端薄宿主入口 | Kimi | GLM | P01-6 | todo | todo | todo | N/A | Web `/design-schemes` route 与 Desktop view/nav 尚未挂载；两端 `hasDesignSchemes=false` 是诚实阻断，待各自 lifecycle 可用后开启 |
...
|---|---|---|---|---|---|---|---|---|
| B00 | 主代理 | — | verify | verify | verify | N/A | verify | 当前树 `pnpm run check` 已通过（179 个测试文件、1277 个测试、35 个 Turbo task）;完整 v25 E2E 的原生 macOS fullscreen 受 runner 前台焦点阻断,本轮未提交且无 report/artifact 绑定,仍不能登记为 done/pass;同步截图中 gitignored 临时附件也不替代 durable baseline |
