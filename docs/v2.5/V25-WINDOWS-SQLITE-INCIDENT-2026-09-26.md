...
官网当前文件对应构建源码 `a819fd2`。后续的构建中间文件排除与扫描器修复提交没有进入这个已发布的文件；不会在同一下载地址下静默替换其字节。
...
干净重包候选的 Windows NSIS 内外扫描已经达到 0 findings、0 errors；第二轮冒烟受构建机卡顿影响出现超时及临时目录删除 `EPERM`，因此没有把该候选替换到官网。
