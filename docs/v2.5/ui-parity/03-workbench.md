...
会话式生成的主干(时间线 + Composer + 空态 + 取消/重试/删除回合)已迁,契约化和状态矩阵比旧版更严整(cancelling 中间态、失败错误卡、`isComposing` 保护都是新版做得更好的地方)。但旧工作台是全产品打磨最深的一屏,当前缺口集中在四块:**参考图输入链**(上传/拖拽/粘贴/预览条,生图产品的核心输入)、**结果卡动作组**(下载/存为提示词/微调/多选批量保存全部缺失,新版结果只能看和放大)、**空态品牌动效**(水印呼吸 + 建议横滚,D13 需按「体验一致」原则翻案恢复)、**时间线工程细节**(回到最新钮、消息编辑/复制)。

## 2. 布局对照

| 区块 | 旧版 | 新版 | 判定 |
|---|---|---|---|
| 整体 | 时间线(内容 760px 中轴)+ 贴底 Composer;右侧可展开素材库 Dock(304px) | ✅ 已收口(2026-08-29):时间线内容列 728px 中轴 + **悬浮贴底 Composer**(728px 卡绝对定位、轨道透明、浮起阴影 + 轻透底毛玻璃,时间线以底部留白 172/220px 从卡后滚过,承旧 floating 布局) | 收口;素材库 Dock 随域暂缓 |
| 空态 | `WorkbenchEmptyState`:水印背景 + 品牌锁定区(mark 96px + tagline)+ 三条横滚建议 + 内联 Composer,整体 `clamp(72px,16vh,140px)` 顶距 | ✅ 已收口(2026-08-29):水印/mark 96px(移动 72px)/横滚建议/`clamp` 顶距全承旧;内联 Composer 20px 品牌焦点外框(移动 16px);矮视口(≤560px 高)建议让位、顶距收紧(承旧 11 §10.3) | 收口 |
| 用户消息 | 左侧头像区无,右对齐气泡 + 气泡上方附件区(Codex 式)+ meta 行(参数摘要/微调来源链) | 右对齐 `rounded-2xl rounded-br-sm bg-primary` 气泡,无 meta 行 | 气泡形一致;**meta 行(比例/质量参数摘要)缺失,P2** |
...
| 控件 | 旧版 | 新版 | 判定 |
|---|---|---|---|
| 「+」菜单 | `WorkbenchContextMenu` 分区菜单:添加图片(主项)/引用提示词/设计方案/GitHub Skill/生成设计方案/从历史创建/寻找方案 | ✅ 已收口:`composer-attach` 菜单(暂只「添加图片」+ 入图方式提示) | 收口;其余项属暂缓域挂点 |
...
| 附件预览条 | `WorkbenchComposerContextTray`:草稿图缩略(可删/可预览)、提示词引用卡(整条/片段)、历史来源 chip、方案 chip、微调目标引用卡 | ✅ 已收口:草稿图 24px chip 条(缩略 16px/单行截断/可删/点击预览,03-C2 解剖) | 收口;引用卡/来源 chip 随各暂缓域 |
...
| 行首 Backspace | 光标在 0 位按 Backspace 依次弹出引用胶囊/指令芯片(Codex 式) | 空输入 Backspace 逐个弹出参考图 chip(03-C2) | 同构;引用胶囊随引用体系回归 |
| 指令建议浮层 | `/` 指令 hints:listbox + 方向键循环 + Enter/Tab 选中 + Esc 关闭,`animate-scale-fade-in` | 无 | 暂缓域(设计方案指令),挂点登记 |
| 比例 | `WorkbenchRatioPicker`:预设 + **自定义比例输入**(547 行) | ✅ 已收口(2026-08-29):承旧全集 11 档(auto 殿后)+ 形状预览触发钮(26px 基准色板 + mono 值)+ 368px 三列网格菜单(预览卡/勾选/打开聚焦当前项/方向键 ±1 环绕 + Home/End) | 收口;自定义比例 D7 暂缓 |
...
| 超限提示 | 合成提示词(正文+引用)超限时红字,三种上下文文案 | maxLength 截断,无合成概念 | 等价(无引用时不会超) |
...
| 用户消息激活 | 点击气泡(排除按钮/链接)或 Enter/Space 激活 → 显示「复制/编辑」动作;点空白或 Esc 关闭 | ✅ 已收口(2026-08-29):交互口径升级为 hover/focus 渐显常驻动作组(§8-I2,与助手动作行同构,免激活状态机);复制写剪贴板,编辑回填整份请求参数(prompt/negative/比例/质量)并聚焦置尾,生成中禁用 | 收口 |
...
> 暂缓域挂点(不排任务,防丢):工作模式 tab、指令建议浮层、提示词引用卡、历史来源 chip、方案/Skill preface 对话、微调目标引用、素材库 Dock、豆包托管文案。恢复次序由各域排卡决定。
...
| 编号 | 级 | 增益 | 规格 |
|---|---|---|---|
| 03-C1 | C1 | 时间线→Composer 渐隐过渡 | Composer 上缘 16–24px 渐隐遮罩(mask 或同色 gradient),内容滚过时不硬切(Codex 同法);静态遮罩,不受动效闸门影响 |
