...
旧列表用 `@tanstack/react-virtual`(行高 86/88 估算,overscan 5)支撑全量本地记录;新版无限查询分页 + 「加载更多」。2026-09-06 补滚动哨兵(`history-load-sentinel`,IntersectionObserver 提前一屏取下一页,不支持的环境退化到手动按钮),首屏与连续浏览都不再需要点按钮。**虚拟化本体仍未做**:`@tanstack/react-virtual` 只在 `apps/desktop` 声明,要在 features 用必须给 `packages/features/package.json` 加依赖 —— 留作独立卡(阈值 ~150 行,落地时按 05-C4 复核筛选淡切实现)。
...
| P2(余) | 大列表虚拟化(>150 行) | 见 §6:先落了滚动哨兵自动加载;虚拟化需先给 `packages/features` 加 `@tanstack/react-virtual` 依赖 |
