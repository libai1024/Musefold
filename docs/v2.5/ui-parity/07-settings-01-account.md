...
账号是设置域里**迁移最完整**的分区:账密登录/注册、积分展示、兑换码、退出登录(带确认)、云同步开关全部就位,且形态按 D9(内联表单,四端同一份)重设计获批。差值集中在:**注册确认密码**、**登出后用户名预填**、**兑换错误的错误码文案表**、**云同步冲突处理 UI**、以及整个**豆包体验通道**(冻结面挂点)。
...
| 字段 | 用户名、密码、**注册时确认密码** | 用户名、密码 | **P1:注册缺确认密码**——旧版有表单级「两次输入的密码不一致」红字(按钮 disabled 拦不住 Enter 提交,须可见反馈),防呆语义要带回 |
| 描述文案 | 「推荐通道:一次登录,Agent 与生图模型自动配置,无需管理 API Key。」 | 简化版卡描述 | P3 文案对齐 |
...
| 兑换码 | 输入 + 兑换;**错误码翻译表**(`ACCOUNT/REDEEM_INVALID` → 「兑换码无效或已使用」/ 服务不可用 → 「稍后重试」);成功 toast 带到账积分 | 输入 + 兑换 + 成功 toast 到账积分 | 主干一致;**P2 错误码文案表迁移**(现在可能裸抛 message) |
...
| 手动同步 | 「立即同步 / 同步中…」钮 | 状态行(同步中/有冲突/上次同步时间) | **P2:「立即同步」手动触发钮缺失** |
| 冲突 | **「需要处理的同步冲突」列表区**(逐条处理) | 状态显示「有冲突」但无处理 UI | **P1 缺口**:冲突可见不可解,数据一致性闭环断裂;冲突解决 UI(本地/云端二选一逐条)需排卡 |
...
| 云同步卡 | 状态行骨架 | 「同步状态读取失败,请重试」 | 未登录=开关禁用 + 警示条 | 开关 + 状态行(idle/syncing/conflict/error)+ 上次同步时间 |
...
| 优先级 | 任务 | 验收要点 |
|---|---|---|
| P1 | 注册确认密码字段 + 不一致红字(Enter 提交路径也拦截) | 不一致时提交被阻;单测覆盖 |
| P1 | 云同步冲突处理 UI:冲突列表 + 逐条「保留本地/保留云端」 | 处理后状态回 idle;api/桥联动测试 |
| P2 | 「立即同步」手动钮(syncing 态禁用);兑换错误码文案表迁移 | 错误码→中文文案单测 |
...
| 0701-C3 | C2 | 冲突处理 UI 的工艺基线(挂靠 §7 冲突 P1) | 冲突列表 = 00 §5.2 行解剖(标题 + 本地/云端时间戳 tabular + 行尾「保留本地/保留云端」双文字钮);逐条处理后行 `--dur-base` 离场;不做全屏对比视图(≤5 条的常见规模不值) |
