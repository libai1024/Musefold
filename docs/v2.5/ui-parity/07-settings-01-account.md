...
账密登录/注册、确认密码、积分、兑换、退出、同步冲突处理和登录设备治理已落地；形态按D9使用共享内联表单。09-21反向审计发现旧“服务公告/全部已读”未接入，现已补契约、公开公告读取、双宿主Gateway与共享卡片，局部三形态联合验收13项通过，未关闭整页parity。豆包只保留已登记的薄入口，专属网页渲染仍冻结；不再将已实现的同步冲突UI列为缺口。
...
| 描述文案 | 「推荐通道:一次登录,Agent 与生图模型自动配置,无需管理 API Key。」 | 简化版卡描述；生图与Agent文本连接使用独立设置卡 | **当前范围**：`AgentConnectionsPanel`已接主进程文本连接，不能再以旧稿“没有Agent连接UI”为现状。独立通道语义见UI-SPEC §7.2；不凭文案保证任何指定模型可用。 |
| 提交态 | 「登录中…/注册中…」+ disabled | 同 | 一致 |
| 错误 | 表单内红字(保留输入) | `account-auth-error` 红字 | 一致(I4) |
| 预填 | **登出/会话失效后回表单预填上次用户名**,只需重输密码 | ✅ 登出后预填 `account.status.username`(zustand + sessionStorage,不进偏好) | 收口 |
...
| 兑换 | 输入 + 兑换;**错误码翻译表**(`ACCOUNT/REDEEM_INVALID` → 「兑换码无效或已使用」/ 服务不可用 → 「稍后重试」);成功 toast 带到账积分 | 输入 + 兑换 + 成功 toast 到账积分;**错误码文案表**(`account/error-messages.ts`);B6 若有额度失败恢复意图,toast 追加「正在重试生成」并走 `generation.retry` | ✅ 收口 |
...
| 服务公告 | 纯文本公告与“全部已读”，内容ID保存在本设备 | 登录设备下的共享公告卡，卡组间距24px；独立读取、手动刷新/失败重试、显式全部已读及本设备持久化；兼容含原始首尾空白的旧ID别名，新记录按账号/API服务/公告发行服务隔离 | 09-21局部评审 ship；PC/移动公告、新PID Electron持久化及旧app/file origin迁入已验；移动账号快照已核图同步，13项联合通过，最终全量与整页parity仍开放 |

**服务公告局部证据与边界（09-21）**：本卡沿用既有中性token、Card/Button和系统字体；正文/动作可换行、移动按钮至少44px，无新增图片资产。`.impeccable/review/notices-v25/` 的桌面、移动、375/667px、深色及macOS七张截图与实际组件经独立评审，结论 `ship` 仅覆盖新增公告卡和接入间距；深色聚焦截图不构成实际Tab焦点指示或计算对比度的完整验收。[detector](../../../tests/v25/.results/live25-20260921/notices-design-detect.json) 为 `[]`，[check记录](../../../tests/v25/.results/live25-20260921/notices-check-5.log) 为38/38成功；组件11项、契约/传输/桥接17项通过，真实API专项25项中公告4项、会话分页21项。原第2轮12通过/1快照失败保留；仅对已核图的移动账号24px间距差异同步基线后，[第3轮宿主专项](../../../tests/v25/.results/live25-20260921/notices-hosts-3.json) 13P/0F/0S/0flaky，48.7秒，零重试、不更新快照。以上不代表最终同源全量、远端付费流程、整页parity或完整迁移完成。
...
| ~~P2~~ | ~~兑换错误码文案表迁移~~ ✅ 已收口(2026-09-06):`account/error-messages.ts` 覆盖 AUTH_*/ACCOUNT_*/OAuth + 桌面 `ACCOUNT_SERVICE_UNAVAILABLE` | 错误码→中文文案单测 |
| ~~P2~~ | ~~登出后用户名预填~~ ✅ 已收口(2026-09-06):登出后预填上次用户名、密码清空并聚焦 | 登出→表单用户名保留 |
