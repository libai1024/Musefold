...
## 当前非 Windows 核对（2026-09-21，最终回归进行中）

本表按当前入口、实现与测试重新核对，替代下方历史百分比作为进度入口。源码实现和已有专项/实网结果不等于本轮全量终态；当前候选完整check已通过，最终三形态E2E与新安装包验证仍在运行，详见[真实验收记录](../V25-REAL-ACCEPTANCE-2026-09-21.md)。

| 产品面 | 当前可达实现 | 当前核对依据与边界 |
|---|---|---|
| 壳、侧栏、首启 | 共享AppShell、调宽/收起/移动抽屉、日期分组、置顶/改名/归档/未读、身份菜单、首启连接引导 | `SessionListPanel`及壳/首启的Web双视口、Electron测试；`electron.shell`确有原生macOS全屏和拖拽区域验证。Windows真实几何仍延期，不能由darwin跳过分支代替 |
