...
vi.mock('../smartSets', () => ({ registerSmartSetHandlers: registrations.smartSet }));
vi.mock('../folders', () => ({ registerFolderHandlers: registrations.folder }));
vi.mock('../tags', () => ({ registerTagHandlers: registrations.tag }));
vi.mock('../providers', () => ({ registerProviderHandlers: registrations.provider }));
vi.mock('../settings', () => ({ registerSettingsHandlers: registrations.settings }));
vi.mock('../images', () => ({ registerImageHandlers: registrations.image }));
vi.mock('../history', () => ({ registerHistoryHandlers: registrations.history }));
vi.mock('../system', () => ({ registerSystemHandlers: registrations.system }));
vi.mock('../share', () => ({ registerShareHandlers: registrations.share }));
vi.mock('../workbench-sessions', () => ({ registerWorkbenchSessionHandlers: registrations.workbenchSession }));
vi.mock('../skill-runtime', () => ({ registerSkillRuntimeHandlers: registrations.skillRuntime }));
vi.mock('../design-scheme', () => ({ registerDesignSchemeHandlers: registrations.designScheme }));
vi.mock('../automation', () => ({ registerAutomationHandlers: registrations.automation }));
vi.mock('../account', () => ({ registerAccountHandlers: registrations.account }));
