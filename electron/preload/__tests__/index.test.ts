...
describe('electron preload api bridge', () => {
  ...
  it('exposes design scheme, Skill runtime, image and system APIs through contextBridge', async () => {
    electronMock.ipcRenderer.invoke.mockResolvedValue({ ok: true, data: { ok: true } });
    ...
    expect(electronMock.ipcRenderer.invoke).toHaveBeenCalledWith(IPC.SKILL_RUNTIME_CANCEL, 'skill-op-1');
    await api.image.pickLocal();
    expect(electronMock.ipcRenderer.invoke).toHaveBeenCalledWith(IPC.IMAGE_PICK_LOCAL);
    const staged = { bytes: Uint8Array.from([1, 2, 3]), name: 'pasted.png', mimeType: 'image/png' };
  ...
  it('returns cleanup functions for all event subscriptions', async () => {
    ...
    const skillListener = electronMock.ipcRenderer.on.mock.calls.find(([channel]) =>
      channel === IPC.SKILL_RUNTIME_EVENT,
    )?.[1] as (event: unknown, payload: unknown) => void;
    const skillEvent = { executionId: 'skill-op-1', kind: 'state', state: 'preparing' };
    ...
    const imageListener = electronMock.ipcRenderer.on.mock.calls.find(([channel]) =>
      channel === IPC.IMAGE_PROGRESS,
    )?.[1] as (event: unknown, payload: unknown) => void;
    stopImage();
    expect(electronMock.ipcRenderer.removeListener).toHaveBeenCalledWith(
      IPC.IMAGE_PROGRESS,
    ...
    const shareListener = electronMock.ipcRenderer.on.mock.calls.find(([channel]) =>
      channel === IPC.SHARE_INCOMING,
    )?.[1] as (event: unknown, payload: unknown) => void;
    stopShare();
    expect(electronMock.ipcRenderer.removeListener).toHaveBeenCalledWith(
      IPC.SHARE_INCOMING,
    ...
    const maximizeListener = electronMock.ipcRenderer.on.mock.calls.find(([channel]) =>
      channel === 'window:maximizeChanged',
    )?.[1] as (event: unknown, payload: unknown) => void;
    stopMaximize();
    expect(electronMock.ipcRenderer.removeListener).toHaveBeenCalledWith(
    ...
    const fullscreenListener = electronMock.ipcRenderer.on.mock.calls.find(([channel]) =>
      channel === 'window:fullscreenChanged',
    )?.[1] as (event: unknown, payload: unknown) => void;
    stopFullscreen();
    expect(electronMock.ipcRenderer.removeListener).toHaveBeenCalledWith(
  ...
  it('delivers main-process diagnostics pushed over the dedicated channel', async () => {
    ...
    const listener = electronMock.ipcRenderer.on.mock.calls.find(([channel]) =>
      channel === IPC.DIAGNOSTICS_ERROR,
    )?.[1] as (event: unknown, report: unknown) => void;
    expect(listener).toBeTypeOf('function');
    const report = { id: 'diag-1', process: 'main', source: 'main-process', error: { message: 'boom' } };
    listener({}, report);
