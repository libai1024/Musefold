...
export type AutomationRouteHandler = (
...
function readBody(
  ...
  return new Promise((resolve, reject) => {
    ...
    request.on('data', (chunk: Buffer | string) => {
      ...
      bytes += buffer.length;
      if (bytes > maxBytes)
        fail(new AutomationError('REQUEST_TOO_LARGE', '请求体超过大小限制', 413, { maxBytes }));
    ...
    request.on('end', () => {
      ...
      try {
        ...
        if (jsonDepth(parsed) > maxJsonDepth)
          return reject(
            new AutomationError('JSON_TOO_DEEP', 'JSON 嵌套层级超过限制', 400, {
              maxDepth: maxJsonDepth,
            }),
          );
        resolve(parsed);
      } catch {
        reject(new AutomationError('INVALID_JSON', '请求体不是有效 JSON', 400));
      }
    });
  });
}
  ...
  for (const [key, route] of Object.entries(routes)) {
    ...
    for (let index = 0; index < patternSegments.length; index += 1) {
      ...
      if (pattern.startsWith(':'))
        params[pattern.slice(1)] = decodeURIComponent(pathSegments[index]);
      else if (pattern !== pathSegments[index]) {
        matched = false;
        break;
      }
    }
...
export function createAutomationServer(options: AutomationServerOptions): AutomationServer {
  ...
  if (host !== '127.0.0.1' && host !== '::1')
    throw new AutomationError('LOOPBACK_ONLY', '控制面只能绑定 loopback 地址', 400, { host });
  ...
  const generationLimiter = new SlidingWindowLimiter(
    options.generationRateLimit ?? DEFAULT_GENERATION_RATE_LIMIT,
  );
  ...
  const health = () => {
    ...
    try {
      snapshot = options.core.status.snapshot();
    } catch (error) {
      logger.warn('health snapshot unavailable', error);
    }
    return {
      connected: true,
      owner: options.owner,
  ...
  const handler = async (request: IncomingMessage, response: ServerResponse) => {
    ...
    try {
      if (request.headers.origin !== undefined)
        throw new AutomationError('ORIGIN_NOT_ALLOWED', '控制面不接受带 Origin 的浏览器请求', 403);
      const provided = bearerToken(
        typeof request.headers.authorization === 'string'
          ? request.headers.authorization
          : undefined,
      );
      if (!provided || !tokenEquals(token, provided))
        throw new AutomationError('UNAUTHORIZED', '缺少或无效的控制面 token', 401);
      ...
      const body =
        method === 'GET' || method === 'HEAD'
          ...
          : await readBody(
              request,
              path.startsWith(`/${AUTOMATION_API_VERSION}/uploads`)
                ? (options.uploadBodyLimit ?? DEFAULT_UPLOAD_BODY_LIMIT)
                : (options.requestBodyLimit ?? DEFAULT_REQUEST_BODY_LIMIT),
      ...
      const result = await route({
        ...
        body,
        params,
        json: (value, responseStatus = 200) => writeJson(response, value, responseStatus),
      });
      ...
      writeJson(response, envelope, status);
    } finally {
      void audit({
  ...
  return {
    ...
    async start() {
      ...
      currentInfo = {
        ...
        port: actualPort,
        token,
        discoveryPath: discoveryFilePath(options.dataDir),
      };
      try {
        writeDiscoveryFile(options.dataDir, document(actualPort, startedAt));
      } catch (error) {
      ...
      logger.info('automation server listening', { host, port: actualPort, owner: options.owner });
      return currentInfo;
    },
    async stop() {
    ...
    rotateToken() {
      token = generateToken();
      if (currentInfo) {
        currentInfo = { ...currentInfo, token };
        writeDiscoveryFile(
          options.dataDir,
          document(currentInfo.port, startedAt ?? clock().toISOString()),
        );
      }
