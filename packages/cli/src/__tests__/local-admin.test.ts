...
describe('CLI 本地专属命令', () => {
  ...
  it('provider add --use：经质询通道创建', async () => {
    ...
    expect(ops.createProvider).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'CLI站', isActive: true }),
    );
  });
  ...
  it('provider set-key --from-env：密钥不经 argv', async () => {
    const io = capture();
    const code = await run(
      ['provider', 'set-key', 'prov-cli', '--from-env', 'TEST_MUSEFOLD_KEY'],
      io,
      { TEST_MUSEFOLD_KEY: 'sk-from-env' },
    );
    expect(code).toBe(EXIT.OK);
  ...
  it('backup now / list / restore --force + export', async () => {
    ...
    expect(ops.exportLibrary).toHaveBeenCalledWith(
      expect.objectContaining({ targetPath: '/tmp/out.json' }),
    );
  });
