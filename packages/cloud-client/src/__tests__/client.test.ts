...
describe('Musefold Cloud client', () => {
  it('exchanges the relay token once and uses only the opaque desktop session afterwards', async () => {
    ...
    expect(exchangeHeaders.get('Authorization')).toBe('Bearer relay-management-token');
    expect(syncHeaders.get('Authorization')).toBe(`Bearer ${desktopSession.sessionToken}`);
    expect(syncHeaders.get('X-Musefold-CSRF')).toBe(session.csrfToken);
    expect(JSON.stringify(fetchImpl.mock.calls[1])).not.toContain('relay-management-token');
  });
  ...
  it('keeps CSRF state in memory and attaches it to mutations', async () => {
    ...
    expect(new Headers(fetchImpl.mock.calls[1]?.[1]?.headers).get('X-Musefold-CSRF')).toBe(
      session.csrfToken,
    );
  ...
  it('binds desktop pull requests to the registered device when provided', async () => {
    const fetchImpl = vi
      .fn<typeof fetch>()
      .mockResolvedValue(Response.json({ changes: [], nextCursor: '12', hasMore: false }));
    const client = createMusefoldCloudClient('/api/musefold/v1', { fetchImpl });
  ...
  it('maps the canonical API error envelope', async () => {
    ...
    const client = createMusefoldCloudClient('https://cloud.example/api/musefold/v1', {
      fetchImpl,
    });
  ...
  it('exposes retry, soft-delete, and restore generation mutations', async () => {
    ...
    await expect(client.retryGeneration(generationJob.id, 'retry-key-0001')).resolves.toMatchObject(
      {
        id: retried.id,
        parentRunId: generationJob.id,
      },
    );
    await expect(client.deleteGeneration(generationJob.id)).resolves.toMatchObject({
      deletedAt: deleted.deletedAt,
    });
    await expect(client.restoreGeneration(generationJob.id)).resolves.toMatchObject({
      deletedAt: null,
    });
  ...
  it('parses chunked generation SSE and resumes after the supplied cursor', async () => {
    ...
    await client.streamGenerationEvents(generationJob.id, 10, (event) => {
      events.push(event);
    });
    ...
    expect(new Headers(fetchImpl.mock.calls[0]?.[1]?.headers).get('Accept')).toBe(
      'text/event-stream',
    );
    expect(new Headers(fetchImpl.mock.calls[0]?.[1]?.headers).get('Last-Event-ID')).toBe('10');
  });
  ...
  it('loads an individual workbench session for refresh recovery', async () => {
    const fetchImpl = vi.fn<typeof fetch>().mockResolvedValue(Response.json(workbenchSession));
    const client = createMusefoldCloudClient('/api/musefold/v1', { fetchImpl });

    await expect(client.getWorkbenchSession(workbenchSession.id)).resolves.toEqual(
      workbenchSession,
    );
    expect(fetchImpl.mock.calls[0]?.[0]).toBe(
      `/api/musefold/v1/workbench/sessions/${workbenchSession.id}`,
