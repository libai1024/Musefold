 ...
 * - 本文件只定义输出形状；调用次数、费用授权和重试资格由各宿主持久执行层决定。
 * - 保留桌面模型输出的默认值与未知字段剥离行为；落库仍须校验最终 revision 文档。
 */
import { z } from 'zod';
import {
...
/** Pure role inputs. Hosts validate provenance and freeze source bytes before constructing these. */
export const analystInputSchema = z.object({
  brief: z.string(),
  repositoryLabel: z.string(),
  textFiles: z.array(z.object({ path: z.string(), text: z.string() })),
...
export const compilerInputSchema = z.object({
  /** Preserved assets, not visual input to a text-only model. */
  uploadedImageCount: z.number().int().min(0).max(64).optional(),
  brief: z.string(),
  repositoryLabel: z.string().optional(),
  analystReport: analystReportSchema.optional(),
  additionalRepositories: z
    .array(
      z.object({
        repositoryLabel: z.string(),
        analystReport: analystReportSchema,
      }),
    )
    .optional(),
  historyContext: z
...
/** Prompt text is not a model call, an authorization, or a persisted result. */
export const designSchemeRolePromptSchema = z
  .object({ system: z.string(), user: z.string() })
  .strict();
export type DesignSchemeRolePrompt = z.infer<typeof designSchemeRolePromptSchema>;

/** Reviser needs only the editable projection, not storage, history or credentials. */
export const reviserInputSchema = z.object({
  instruction: z.string(),
  document: designSchemeRevisionDocumentSchema.pick({
    name: true,
    summary: true,
    fidelity: true,
    inputs: true,
...
export type ReviserInput = z.infer<typeof reviserInputSchema>;
