...
describe('0017_cloud_prompt_sync', () => {
  it('creates account-isolated sync state, outbox, and conflict tables', () => {
    ...
    expect(
      (db.pragma('table_info(cloud_entity_state)') as Array<{ name: string }>).map(
        (column) => column.name,
      ),
    ...
    expect(() => insertAccount.run('2', 'two', 'device-2', 'Mac two', 1)).toThrow();
    ...
    expect(db.prepare('SELECT owner_id, operation FROM cloud_sync_outbox').get()).toEqual({
      owner_id: '1',
      operation: 'create',
    });
  });
