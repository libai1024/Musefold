...
describe('0001_initial v0.3.0 library seed', () => {
  it('creates only current library examples and indexes them for search', () => {
    ...
    const folders = db.prepare('SELECT name FROM folders ORDER BY sort_order').all() as Array<{
      name: string;
    }>;
    ...
    expect(
      (
        db
          .prepare("SELECT COUNT(*) AS total FROM prompts_fts WHERE prompts_fts MATCH 'cinematic'")
          .get() as { total: number }
      ).total,
    ).toBeGreaterThan(0);
