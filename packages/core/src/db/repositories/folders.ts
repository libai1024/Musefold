...
export const foldersRepo = {
  list(parentId?: string): Folder[] {
    const db = getDb();
    const rows = parentId
      ? db.prepare('SELECT * FROM folders WHERE parent_id = ? ORDER BY sort_order, name').all(parentId)
      : db.prepare('SELECT * FROM folders WHERE parent_id IS NULL ORDER BY sort_order, name').all();
  ...
  create(f: NewFolder): Folder {
    ...
    const maxOrder = db.prepare('SELECT COALESCE(MAX(sort_order), -1) AS m FROM folders WHERE parent_id IS ?').get(f.parentId ?? null) as { m: number };
    db.prepare(
  ...
  update(id: string, patch: Partial<Folder>): Folder {
    ...
    if (patch.name !== undefined) {
      db.prepare('UPDATE folders SET name = ? WHERE id = ?').run(patch.name, id);
    }
    if (patch.parentId !== undefined) {
      ...
      if (patch.parentId) {
        ...
        while (cur) {
          ...
          if (!c) break;
          if (c.parentId === id) throw new Error('不能把文件夹移动到自己的子文件夹下');
          cur = c.parentId;
        }
      }
      db.prepare('UPDATE folders SET parent_id = ? WHERE id = ?').run(patch.parentId, id);
    }
