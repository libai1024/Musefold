...
export const tagsRepo = {
  list(group?: TagGroup): Tag[] {
    const db = getDb();
    const rows = group
      ? db.prepare('SELECT * FROM tags WHERE tag_group = ? ORDER BY name').all(group)
  ...
  update(id: string, patch: Partial<Tag>): Tag {
    const db = getDb();
    if (patch.name !== undefined) db.prepare('UPDATE tags SET name = ? WHERE id = ?').run(patch.name, id);
    if (patch.tagGroup !== undefined) db.prepare('UPDATE tags SET tag_group = ? WHERE id = ?').run(patch.tagGroup, id);
    if (patch.color !== undefined) db.prepare('UPDATE tags SET color = ? WHERE id = ?').run(patch.color, id);
    return this.get(id)!;
  },
