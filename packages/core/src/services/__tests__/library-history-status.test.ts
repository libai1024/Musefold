...
describe("LibraryService", () => {
  it("create → search 全链路：新建提示词可被检索到", () => {
    const created = library.create({
      title: "赛博街景",
      content: "cyberpunk street, neon rain",
    });
...
describe("HistoryService", () => {
  ...
  it("get 返回详情并附带提示词引用；未命中返回 null", () => {
    ...
    expect(detail?.promptReferences[0]).toMatchObject({
      title: "引用源",
      scope: "full",
    });
