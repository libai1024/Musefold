...
describe('DesktopSyncRepository', () => {
  it('builds a cloud-safe outbox and compacts edits into one stable mutation', () => {
    insertLocalPrompt();
    expect(repository.enqueue(ownerId, 'prompt', 'prompt-local', 'create')).toBe(true);
    ...
    db.prepare('UPDATE prompts SET content = ? WHERE id = ?').run('第二次本地编辑', 'prompt-local');
    ...
    db.prepare('UPDATE prompts SET deleted_at = 2 WHERE id = ?').run('prompt-local');
    expect(repository.enqueue(ownerId, 'prompt', 'prompt-local', 'delete')).toBe(false);
  ...
  it('applies cloud snapshots without creating echo mutations and refreshes FTS', () => {
    ...
    repository.applyBootstrapSnapshot(ownerId, 'prompt', remotePrompt('来自云端的正文', 1));
  ...
  it('keeps bootstrap collisions as conflicts instead of seeding an overwrite', () => {
    ...
    repository.applyBootstrapSnapshot(ownerId, 'prompt', remotePrompt('云端不同正文', 1));
  ...
  it('preserves both snapshots during a conflict and supports all prompt resolutions', () => {
    repository.applyBootstrapSnapshot(ownerId, 'tag', remoteTag());
    repository.applyBootstrapSnapshot(ownerId, 'prompt', remotePrompt('共同基线', 1));
    db.prepare("UPDATE prompts SET content = '本地修改' WHERE id = 'prompt-cloud'").run();
    ...
    db.prepare("UPDATE prompts SET content = '再次本地修改' WHERE id = 'prompt-cloud'").run();
    repository.enqueue(ownerId, 'prompt', 'prompt-cloud', 'update');
    ...
    repository.applyPushResult(ownerId, repository.listReadyMutations(ownerId)[0]!, {
    ...
    db.prepare("UPDATE prompts SET content = '要保留的本地版本' WHERE id = 'prompt-cloud'").run();
    repository.enqueue(ownerId, 'prompt', 'prompt-cloud', 'update');
  ...
  it('commits a pull page and cursor atomically', () => {
    ...
    expect(() => repository.applyPullPage(ownerId, [invalidChild], '9')).toThrow();
