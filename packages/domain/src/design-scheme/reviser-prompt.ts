import type { ReviserInput, DesignSchemeRolePrompt } from '@musefold/contracts/design-scheme-model';
...
2. 保持结构合法：文本输入的 variable 与 promptProgram 模板中的 {{变量名}} 必须一一对应；
   ...
   每个模块的 variables 只列该 template 中实际使用的变量；没有变量时使用 []。
   文本输入不得带 imageRole；图片输入必须指定合法 imageRole，且不得带 variable。可选字段不适用时省略，不填空字符串或 null。
...
export function buildReviserPrompt(input: ReviserInput): DesignSchemeRolePrompt {
