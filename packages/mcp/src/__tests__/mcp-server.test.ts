...
beforeAll(async () => {
  ...
  const mcpImagePaths = [1, 2, 3, 4].map((index) =>
    join(root, index === 1 ? 'out.png' : `out-${index}.png`),
  );
...
describe('musefold-mcp', () => {
  ...
  it('search_prompts / list_providers（密钥红线）', async () => {
    ...
    const found = parseText(
      await client.callTool({ name: 'search_prompts', arguments: { query: '青鸾照水样张' } }),
    );
    // FTS 中文分词可能命中种子示例，只断言目标提示词在结果中
    expect(found.prompts.some((prompt: { title: string }) => prompt.title === '青鸾照水样张')).toBe(
      true,
    );
  ...
  it('generate_image：wait=true 阻塞到完成并返回 ResourceLink', async () => {
    ...
    const result = await client.callTool({
      name: 'generate_image',
      arguments: { prompt: '一张 MCP 测试图', wait: true },
    });
    const payload = parseText(result as never);
    expect(payload.status).toBe('success');
    expect(payload.costPoints).toBe(20);
    const links = (result as { content: Array<{ type: string; uri?: string }> }).content.filter(
  ...
  it('generate_image：wait=false 提交即返回 jobId，wait_for_generation 一次等待终态', async () => {
    ...
    const submitted = parseText(
      await client.callTool({
        name: 'generate_image',
        arguments: { prompt: '轮询路径', wait: false },
      }),
    );
    expect(submitted.jobId).toBeTruthy();
    const detail = parseText(
      await client.callTool({ name: 'wait_for_generation', arguments: { jobId: submitted.jobId } }),
    );
  ...
  it('安全配置工具只传非敏感草稿，并可选择已配置 Provider', async () => {
    ...
    const selected = parseText(
      await client.callTool({ name: 'select_provider', arguments: { providerId: 'prov-mcp' } }),
    );
