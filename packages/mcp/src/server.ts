...
function addTool(
  ...
  handler: (args: any, extra: ToolHandlerExtra) => Promise<unknown>,
): void {
...
function addPrompt(
  ...
  (server.registerPrompt as unknown as (n: string, c: unknown, h: unknown) => void)(
    name,
    config,
    handler,
  );
...
function registerSetupTools(
  ...
  addTool(
    ...
    'get_setup_status',
    {
      title: '检查接入配置',
      description:
        'Read redacted account/provider readiness and the active provider. Never returns usernames, server URLs, key suffixes, tokens, or credentials.',
      inputSchema: {},
      annotations: { readOnlyHint: true },
    },
    async () => {
      try {
        return textResult(await client.setupStatus());
      } catch (error) {
  ...
  addTool(
    ...
    'select_provider',
    {
      title: '选择接入方式',
      description:
        'Switch to an already configured account or relay provider. Reversible and never handles credentials.',
      inputSchema: { providerId: z.string() },
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true },
    },
    async (args) => {
      try {
        return textResult(await client.selectProvider(args.providerId));
      } catch (error) {
        return errorResult(error);
      }
    },
  ...
  addTool(
    ...
    'open_account_setup',
    {
      title: '打开账号配置',
      description:
        'Focus Musefold and open its native login/register form. Ask the user to enter credentials only there; never ask for or pass a username or password in chat.',
      inputSchema: { mode: z.enum(['login', 'register']).optional() },
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true },
    },
    async (args) => {
      try {
        return textResult(await client.openAccountSetup(args.mode));
      } catch (error) {
        return errorResult(error);
      }
    },
  ...
  addTool(
    ...
    'open_provider_setup',
    {
      title: '打开中转站配置',
      description:
        'Focus Musefold and open its native provider form, optionally prefilling non-secret fields. The user must enter the API key and test connectivity inside Musefold.',
      inputSchema: {
    ...
    async (args) => {
      try {
        return textResult(await client.openProviderSetup(args));
      } catch (error) {
        return errorResult(error);
      }
    },
...
function textResult(
  payload: unknown,
  extra: { isError?: boolean; links?: Array<{ uri: string; name: string }> } = {},
) {
  return {
    ...
    return textResult(
      {
        error: {
          ...
          retriable: error.code === 'NOT_CONNECTED',
        },
      },
      { isError: true },
    );
      ...
      register(server, client) {
        addTool(
          ...
          'search_prompts',
          {
            title: '检索提示词库',
            description:
              'Full-text search over the Musefold prompt library (including quick slips). Read-only.',
            inputSchema: {
          ...
          async (args) => {
            try {
              return textResult(
                await client.prompts({ query: args.query, source: args.source, limit: args.limit }),
              );
            } catch (error) {
              return errorResult(error);
            }
        ...
        addTool(
          ...
          async (args) => {
            try {
              return textResult(await client.prompt(args.id));
            } catch (error) {
              return errorResult(error);
            }
          },
        ...
        if (level('write')) {
          addTool(
            ...
            'save_prompt',
            {
              title: '保存提示词',
              description:
                'Save a good prompt back into the Musefold library (goes to the library, recoverable via trash).',
              inputSchema: {
                ...
                note: z
                  ...
                  .max(500)
                  .optional()
                  .describe('Provenance note, e.g. "via Claude Code 2026-08-13"'),
              },
              annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
            },
            async (args) => {
              try {
                return textResult(await client.savePrompt(args));
              } catch (error) {
                return errorResult(error);
              }
            },
      ...
      register(server, client, options) {
        ...
        addTool(
          ...
          'list_provider_models',
          {
            ...
            description:
              'List image models available from one configured provider. Read-only and contains no credentials.',
            inputSchema: { providerId: z.string() },
            annotations: { readOnlyHint: true },
          },
          async (args) => {
            try {
              return textResult(await client.providerModels(args.providerId));
            } catch (error) {
              return errorResult(error);
            }
          },
        ...
        if (level('spend')) {
            ...
            'generate_image',
            {
              ...
              inputSchema: {
                ...
                wait: z
                  .boolean()
                  .optional()
                  .describe('default true; false returns jobId immediately'),
              },
              annotations: {
                readOnlyHint: false,
          ...
          addTool(
            ...
            async (args) => {
              try {
                return textResult(await client.cancelGeneration(args.jobId));
              } catch (error) {
                return errorResult(error);
              }
            },
        ...
        addTool(
          ...
          'get_generation',
          {
            title: '查询生成任务',
            description:
              'Read one current generation snapshot by jobId. For completion notification use wait_for_generation instead of polling.',
            inputSchema: { jobId: z.string() },
            annotations: { readOnlyHint: true },
          },
          async (args) => {
            try {
              return textResult(await client.getGeneration(args.jobId));
            } catch (error) {
              return errorResult(error);
            }
          },
        ...
        addTool(
          ...
          'wait_for_generation',
          {
            title: '等待生成完成',
            description:
              'Wait for a submitted generation job via Musefold events, report progress, and return final image links. Call once after generate_image(wait:false); do not poll.',
            inputSchema: { jobId: z.string() },
      ...
      register(server, client, options) {
        addTool(
          ...
          'list_schemes',
          {
            title: '列出设计方案',
            description:
              'List formal design schemes (visual production skills). Drafts are never exposed. Read-only.',
            inputSchema: {},
            annotations: { readOnlyHint: true },
          },
          async () => {
            try {
              return textResult(await client.schemes());
            } catch (error) {
              return errorResult(error);
            }
          },
        ...
        addTool(
          ...
          'get_scheme',
          {
            title: '读取方案详情',
            description:
              'Fetch scheme detail: input slots (text/image, required flags), constraints, source binding. Read-only.',
            inputSchema: { id: z.string() },
            annotations: { readOnlyHint: true },
          },
          async (args) => {
            try {
              return textResult(await client.scheme(args.id));
            } catch (error) {
              return errorResult(error);
            }
          },
        ...
        addTool(
          server,
          'compile_scheme_prompt',
          {
            title: '编译方案提示词',
            description:
              'Preview the final prompt a scheme would produce for given inputs. Zero cost, read-only.',
            inputSchema: {
              schemeId: z.string(),
        ...
        if (level('spend')) {
          addTool(
            ...
            'run_scheme',
            {
              title: '运行设计方案',
              description:
                'Run a formal design scheme to generate images (costs money; desktop confirmation or budget applies). May take minutes; supports wait:false + get_generation polling.',
              inputSchema: {
                schemeId: z.string(),
                inputs: z.record(z.string(), z.string()).optional(),
                brief: z.string().optional(),
      ...
      register(server, client, options) {
        ...
        addTool(
          ...
          'run_github_skill',
          {
            ...
            description:
              'Fetch a public GitHub visual skill (pinned commit, scripts are NEVER executed) and generate images following its rules. Requires a configured text AI connection; costs money (confirmation/budget applies).',
            inputSchema: {
              url: z.string().url(),
              prompt: z.string(),
              idempotencyKey: IDEMPOTENCY_KEY_FIELD,
      ...
      register(server, client) {
        addTool(
          ...
          'list_history',
          {
            title: '列出生成历史',
            description:
              'List generation history rows (status, cost in points, asset paths). Read-only.',
            inputSchema: {
              limit: z.number().int().min(1).max(50).optional(),
          ...
          async (args) => {
            try {
              return textResult(await client.history(args));
            } catch (error) {
              return errorResult(error);
            }
          },
...
export async function createMusefoldMcpServer(
  ...
  addTool(
    ...
    'musefold_status',
    {
      title: '连接诊断',
      description:
        'Check the connection to the local Musefold app and get capability/data counts. Always available.',
      inputSchema: {},
      annotations: { readOnlyHint: true },
    },
    async () => {
      if (!client) {
        return textResult({
          connected: false,
          guidance:
            '无法连接 Musefold 0.5 桌面控制面。请启动 Musefold，并在“设置 > 自动化”中开启本地控制面，然后重启本 MCP 服务器。桌面账号不会提供给 `musefold serve`。',
        });
      }
      try {
