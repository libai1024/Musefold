...
import { createNewApiClient, NewApiClientError, noticeId, normalizeNewApiUrl } from '../index';
...
describe('shared new-api client', () => {
  ...
  it('parses login access and refresh credentials without exposing them to logs', async () => {
        ...
        new Response(
          ...
          }),
          {
            ...
            headers: {
              ...
              'set-cookie': 'new_api_refresh=refresh-secret; HttpOnly; Path=/',
            },
          },
        ),
    );
    ...
    expect(fetchImpl).toHaveBeenCalledWith(
      'https://api.example.com/api/user/login',
  ...
  it('maps all redemption failures to one non-enumerating error', async () => {
    const client = createNewApiClient('https://api.example.com', {
      fetchImpl: async () =>
        new Response(JSON.stringify({ success: false, message: 'code already used' }), {
          status: 400,
  ...
  it('rewrites 2FA login failures without changing the credentials code', async () => {
    const client = createNewApiClient('https://api.example.com', {
      fetchImpl: async () =>
        new Response(JSON.stringify({ success: false, message: 'please complete 2FA' }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        }),
    });
    ...
    expect(notices).toEqual([
      {
        id: noticeId('维护窗口'),
        content: '维护窗口',
        publishedAt: Date.parse('2026-08-14T10:00:00Z'),
      },
