...
describe('shared generation snapshots', () => {
  ...
  it('orders snapshots without mutating either host collection', () => {
    ...
    expect(sorted.map((item) => item.id)).toEqual(['generation-a', 'generation-b']);
    expect(input.map((item) => item.id)).toEqual(['generation-b', 'generation-a']);
  ...
  it('replaces one snapshot and keeps deterministic timeline order', () => {
    ...
    const snapshots = upsertWorkbenchGenerationSnapshot([newer, older], replacement);
