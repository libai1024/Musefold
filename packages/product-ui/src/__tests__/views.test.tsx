...
import { GenerationHistoryScreen } from '../history/GenerationHistoryScreen';
import { PromptLibraryScreen } from '../library/PromptLibraryScreen';
import { PromptLibraryHeaderActions } from '../library/PromptLibraryHeaderActions';
...
import { WorkbenchSessionList, groupWorkbenchSessions } from '../workbench/WorkbenchSessionList';
...
import { ProductTopbar, productTopbarDisplayTitle } from '../navigation/ProductTopbar';
...
describe('shared product views', () => {
  ...
  it('keeps the compact title rule identical across desktop and web shells', () => {
    ...
    expect(productTopbarDisplayTitle('中文长标题一二三四五六')).toBe('中文长标题一二三四五');
  ...
  it('keeps history inspector navigation deterministic across hosts', () => {
    ...
    expect(historyInspectorReducer(detail, { type: 'open-trash' })).toMatchObject({
      mode: 'trash',
      selectedId: null,
    });
    expect(historyInspectorReducer(detail, { type: 'open-list' })).toMatchObject({
      mode: 'list',
      selectedId: 'history-1',
    });
  });
  ...
  it('lets the desktop shell own the prompt-library page identity', () => {
    const html = renderToStaticMarkup(
      <PromptLibraryScreen
        prompts={[]}
        showPageHeader={false}
        headerAction={<button type="button">新建</button>}
        onUse={() => undefined}
      />,
    );

    expect(html).not.toContain('<h1>提示词库</h1>');
    expect(html).toMatch(/mf-library-toolbar[^>]*>[\s\S]*搜索提示词[\s\S]*新建/);
  });

  it('supports the desktop two-row prompt-library control deck', () => {
    const html = renderToStaticMarkup(
      <PromptLibraryScreen
        prompts={[]}
        showPageHeader={false}
        scopeNavigation={<button type="button">全部 0</button>}
        sectionSummary={<span>提示词 0</span>}
        headerAction={
          <PromptLibraryHeaderActions onCreate={() => undefined} onRefresh={() => undefined} />
        }
      />,
    );

    expect(html).toContain('mf-library-control-deck');
    expect(html).toMatch(/mf-library-control-primary[^>]*>[\s\S]*全部 0[\s\S]*library-search/);
    expect(html).toMatch(
      /mf-library-control-secondary[^>]*>[\s\S]*提示词 0[\s\S]*library-refresh[\s\S]*library-new/,
    );
  });

  it('lets the desktop shell own the generation-history page identity', () => {
    const html = renderToStaticMarkup(
      <GenerationHistoryScreen
        items={[]}
        count={12}
        showPageHeader={false}
        headerAction={<button type="button">成本看板</button>}
        onRefresh={() => undefined}
      />,
    );

    expect(html).not.toContain('<h1>生成历史</h1>');
    expect(html).toContain('12 条记录');
    expect(html).toContain('成本看板');
    expect(html).toContain('aria-label="刷新历史"');
  });

  it('renders prompt details as a closable inspector when requested', () => {
    const html = renderToStaticMarkup(
      <PromptDetailScreen
        prompt={{
          id: 'prompt-inspector',
    ...
    expect(html).toContain('data-layout="inspector"');
    expect(html).toContain('提示词详情');
  ...
  it('shares composer submission and draft status semantics', () => {
    ...
    const saving = renderToStaticMarkup(<WorkbenchComposerSaveStatus status="saving" />);
    const saved = renderToStaticMarkup(<WorkbenchComposerSaveStatus status="saved" />);
  ...
  it('shares generation retry semantics between hosts', () => {
    const idle = renderToStaticMarkup(<GenerationRetryAction onRetry={() => undefined} />);
    const busy = renderToStaticMarkup(<GenerationRetryAction onRetry={() => undefined} busy />);
  ...
  it('shares generation result media and status states', () => {
    ...
    const pending = renderToStaticMarkup(
      <GenerationResultSurface status="pending" progressLabel="42%" footerLabel="生成中" />,
    );
  ...
  it('owns the complete composer control geometry while exposing capability slots', () => {
    ...
    expect(html).toContain('class="mf-workbench-toolbar mf-workbench-composer-toolbar"');
  ...
  it('shares the draft conflict prompt so both hosts resolve drafts identically', () => {
    const html = renderToStaticMarkup(
      <WorkbenchDraftConflictNotice onUseRemote={() => undefined} onKeepLocal={() => undefined} />,
    );
  ...
  it('shares user message action geometry and accessible semantics', () => {
    const html = renderToStaticMarkup(
      <WorkbenchMessageActions onCopy={() => undefined} onEdit={() => undefined} editDisabled />,
    );
  ...
  it('shares the workbench empty state and composer controls', () => {
    ...
    // v2.0(11 §4):品牌锁定区 = 放大的产品 mark + 换行提示语;英文名称退为逐字母
    // 水印背景(workbench-empty-watermark),不再有营销 Hero/CTA,名称不与 mark 并排。
    expect(empty).toContain('data-testid="workbench-empty-brand"');
    expect(empty).toContain('data-testid="workbench-empty-watermark"');
    expect(empty).toContain('class="mf-workbench-empty-watermark-word"');
    // 水印逐字母拆分(错相呼吸 + 逐字母 hover 高亮)。
    expect(empty).toContain('>M</span>');
    expect(empty).toContain('>d</span>');
    expect(empty).not.toContain('workbench-empty-name');
  ...
  it('keeps the page shell shared while exposing host capability slots', () => {
    ...
    expect(html).toContain('data-auxiliary-open="true"');
    expect(html).toContain('--mf-workbench-auxiliary-width:336px');
    expect(html).toContain('class="mf-workbench-primary"');
    expect(html).toContain('class="mf-workbench-stage host-stage"');
    expect(html).toContain('class="mf-workbench-auxiliary"');
  ...
  it('shares the navigation shell and button DOM between hosts', () => {
    ...
    expect(mobileButton).toMatch(/class="[^"]*mf-product-sidebar-nav-button[^"]*nav-button[^"]*"/);
  ...
  it('shares one resizable and responsive sidebar rail between hosts', () => {
    const html = renderToStaticMarkup(
      <ProductSidebarLayout open onOpenChange={() => undefined} sidebar={<aside>导航</aside>}>
        <main>工作区</main>
      </ProductSidebarLayout>,
    );
  ...
  it('shares the compact history action hierarchy between hosts', () => {
    const html = renderToStaticMarkup(
  ...
  it('shares the history list and inspector geometry between hosts', () => {
    ...
    expect(html).toContain('data-testid="history-detail-close"');
    expect(html).toContain('关闭生成详情');
