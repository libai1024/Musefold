...
export const CONNECTED_APP_SCOPES = ['account:read', 'prompts:read', 'skills:read'] as const;
  ...
  return (
    ...
    >
      ...
      <div className="mf-connected-apps-list">
        {!loading &&
          items.map((connection) => {
            ...
            return (
              <article
                className="mf-connected-app-row"
                key={connection.id}
              ...
              >
                <div className="mf-connected-app-heading">
                  ...
                  {hasAllScopes(connection.scopes) ? (
                    <span
                      className="mf-connected-app-all-badge"
                      data-testid="connection-all-capabilities"
                    >
                  ...
                  {CONNECTED_APP_SCOPES.map((scope) => {
                    ...
                    return (
                      <button
                        ...
                        title={
                          enabled
                            ? `收窄：移除${CONNECTED_APP_SCOPE_LABELS[scope]}`
                            : `扩大：授予${CONNECTED_APP_SCOPE_LABELS[scope]}（需密码确认）`
                        }
                ...
                <div className="mf-connected-app-controls">
                  ...
                  <BudgetInput
                    ...
                    onCommit={(value) =>
                      void update(connection.id, { maxPointsPerGeneration: value })
                    }
                    onError={setError}
                  />
                  <BudgetInput
                ...
                <div className="mf-connected-app-footer">
                  <span>
                    今日已用 {connection.spentPointsToday.toLocaleString()} · 已预留{' '}
                    {connection.reservedPointsToday.toLocaleString()} 积分
                  </span>
                  <div>
                    {connection.status !== 'revoked' ? (
                      <Button
                        ...
                        onClick={() =>
                          void update(connection.id, { suspended: connection.status === 'active' })
                        }
                        icon={
                    ...
                    {connection.status !== 'revoked' ? (
                      pendingRevokeId === connection.id ? (
                        <span
                          className="mf-connected-app-revoke-confirm"
                          role="group"
                          aria-label="确认撤销授权"
                        >
                          <span>确认撤销？</span>
                        ...
                        <Button
                          variant="ghost"
                          className="mf-connected-app-action mf-connected-app-action-danger"
                          disabled={busyId === connection.id}
        ...
        {loading ? (
          <div className="mf-connected-app-empty" role="status">
            正在读取连接...
          </div>
        ) : items.length === 0 ? (
          <div className="mf-connected-app-empty">
            ...
            <div className="mf-connected-app-empty-guide">
              <span>
                在 AI 客户端中添加 Musefold MCP 服务器即可连接，新连接默认开放最小只读能力。
              </span>
              {mcpServerUrl ? (
                <Button
                  variant="outline"
      ...
      >
        ...
        >
          <form
            className="mf-connected-app-reauth-form"
            onSubmit={(event) => void submitReauth(event)}
          >
            <div className="mf-connected-app-reauth-heading">
              ...
              <div>
                ...
                <DialogDescription>
                  扩大能力、提高预算或恢复连接前，请再次验证你的账号。
                </DialogDescription>
              </div>
              <IconButton
                className="mf-connected-app-icon-button"
                label="取消重新认证"
                title="取消"
                onClick={() => setPendingReauth(null)}
              >
                <X aria-hidden="true" />
              </IconButton>
            </div>
            ...
            <div className="mf-connected-app-reauth-actions">
              <Button
                variant="secondary"
                className="mf-connected-app-button mf-connected-app-button-secondary"
                onClick={() => setPendingReauth(null)}
              >
                取消
              </Button>
              <Button
                variant="primary"
                className="mf-connected-app-button mf-connected-app-button-primary"
                type="submit"
                disabled={reauthPassword.length < 8}
