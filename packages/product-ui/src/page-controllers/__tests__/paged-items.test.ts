...
describe('page-controller list cache helpers', () => {
  ...
  it('reads both gateway pages and desktop extras arrays', () => {
    ...
    expect(itemsFromQueryData({ items: [{ id: 'b' }], nextCursor: 'c' })).toEqual([{ id: 'b' }]);
    expect(asPagedItems({ items: [{ id: 'b' }], nextCursor: 'c' }).nextCursor).toBe('c');
  });
  ...
  it('upserts without duplicating ids and preserves cache shape', () => {
    ...
    expect(upsertListCache({ items: [{ id: 'a' }], nextCursor: null }, { id: 'b' })).toEqual({
      items: [{ id: 'b' }, { id: 'a' }],
      nextCursor: null,
    });
  });
  ...
  it('drops by id without changing the other cache shape', () => {
    ...
    expect(dropListCache({ items: [{ id: 'a' }, { id: 'b' }], nextCursor: 'z' }, 'b')).toEqual({
      items: [{ id: 'a' }],
      nextCursor: 'z',
    });
  });
  ...
  it('replaces list contents without changing the cache shape', () => {
    ...
    expect(replaceListCache({ items: [{ id: 'a' }], nextCursor: 'n' }, [{ id: 'c' }])).toEqual({
      items: [{ id: 'c' }],
      nextCursor: 'n',
    });
  });
