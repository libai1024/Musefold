...
export interface WorkbenchComposerSurfaceProps
  extends Omit<HTMLAttributes<HTMLDivElement>, 'children' | 'className' | 'ref'> {
  children: ReactNode;
  ...
  surfaceProps?: Omit<HTMLAttributes<HTMLDivElement>, 'children' | 'className' | 'ref'>;
}
...
export function WorkbenchComposerSurface({
  ...
  return (
    ...
    >
      {attachments ? <div className="mf-workbench-composer-attachments">{attachments}</div> : null}
      <div
        {...surfaceAttributes}
        {...surfaceProps}
        ref={surfaceRef}
        className={['mf-workbench-composer-surface', surfaceClassName].filter(Boolean).join(' ')}
