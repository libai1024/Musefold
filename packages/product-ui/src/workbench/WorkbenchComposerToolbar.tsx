 ...
 */
export function WorkbenchComposerToolbar({ children, className }: WorkbenchComposerToolbarProps) {
  return (
    <div className={['mf-workbench-toolbar', className].filter(Boolean).join(' ')}>{children}</div>
  );
}
