  ...
  return (
    <div className="mf-workbench-empty" data-testid="workbench-empty">
      ...
      <div className="mf-workbench-empty-brand" data-testid="workbench-empty-brand">
        ...
        <p className="mf-workbench-empty-tagline" data-testid="workbench-empty-slogan">
          把想法变成可生成的视觉
        </p>
      </div>
      {items.length > 0 ? (
        ...
        >
          {items.map((suggestion) => {
            const sequence = Array.from({ length: DIRECTION_SEQUENCE_COPIES }, () => suggestion);
            return (
      ...
      {composer ? <div className="mf-workbench-empty-composer">{composer}</div> : null}
    </div>
  );
}
