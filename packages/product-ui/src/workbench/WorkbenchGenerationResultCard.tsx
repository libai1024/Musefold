...
export function WorkbenchGenerationResultCard({
  ...
  const startLongPress = (event: ReactPointerEvent<HTMLButtonElement>) => {
    if (!selectionEnabled || !canAct || (event.pointerType === 'mouse' && event.button !== 0))
      return;
    clearLongPress();
  ...
  const hasMediaActions = Boolean(
    onSaveImage || onCopyImage || onRefine || onOpenFolder || onHistory,
  );
        ...
        {onOpenFolder || onHistory ? (
          <DropdownMenu>
            ...
            >
              ...
              {onHistory ? (
                <DropdownMenuItem onSelect={onHistory} data-testid="result-history">
  ...
  return (
    <GenerationResultSurface
      ...
      className={['group', selected || deselecting ? 'border-accent ring-1 ring-accent/45' : '']
        .filter(Boolean)
        .join(' ')}
      status={surfaceStatus}
      imageUrl={imageUrl}
      imageLabel={selectionMode ? (selected ? '取消选择图片' : '选择图片') : '查看大图'}
      imageTitle={
        selectionMode
          ? selected
            ? '取消选择'
