...
export function WorkbenchGenerationSettingsPopover({
  ...
  return (
    <div className="mf-workbench-generation-settings">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <span>{managedLabel ?? `${selectedQuality.label}${count ? ` · ${count}张` : ''}`}</span>
          </Button>
        </PopoverTrigger>
        ...
        >
          <div className="mf-workbench-generation-heading">
            <strong>生成设置</strong>
            <IconButton onClick={() => setOpen(false)} title="关闭" label="关闭生成设置">
              <X aria-hidden="true" />
            </IconButton>
          </div>
          {managed ? (
