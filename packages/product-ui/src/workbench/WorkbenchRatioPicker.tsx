...
function preview(option: WorkbenchRatioOption, className = '', testId?: string) {
...
export function WorkbenchRatioPicker({
  ...
  const selected =
    options.find((option) => option.id === value) ?? customRatio(value) ?? options[0];
  ...
  return (
    <div className={`mf-workbench-ratio-picker ${className ?? ''}`}>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {preview(selected, '', testIdPrefix ? `${testIdPrefix}-selected-preview` : undefined)}
            <span>{selected.id.startsWith('custom:') ? selected.ratio : selected.id}</span>
        ...
        >
          ...
          <div className="mf-workbench-ratio-grid" role="listbox" aria-label="图片比例">
              ...
              return (
                ...
                >
                  {preview(
                    option,
                    'is-card',
                    testIdPrefix ? `${testIdPrefix}-${option.id}-preview` : undefined,
                  )}
                  <span>{option.id}</span>
          ...
          {allowCustomRatio && (
            <div className="mf-workbench-custom-ratio">
              <div>
                ...
                <label>
                  <input
                    value={customWidth}
                    onChange={(event) =>
                      setCustomWidth(event.target.value.replace(/\D/g, '').slice(0, 2))
                    }
                    placeholder="16"
                    inputMode="numeric"
                    aria-label="自定义比例宽"
                  ...
                  <input
                    value={customHeight}
                    onChange={(event) =>
                      setCustomHeight(event.target.value.replace(/\D/g, '').slice(0, 2))
                    }
                    placeholder="10"
                    inputMode="numeric"
                    aria-label="自定义比例高"
              ...
              {customTouched && !customValid ? <small>比例需在 1:4 与 4:1 之间</small> : null}
            </div>
          )}
        </PopoverContent>
      </Popover>
