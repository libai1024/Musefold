...
export function sortWorkbenchGenerationSnapshots<T extends WorkbenchGenerationSnapshot>(
  items: readonly T[],
): T[] {
  return [...items].sort((left, right) => {
    const timeDifference = snapshotTimestamp(left.createdAt) - snapshotTimestamp(right.createdAt);
    return timeDifference || left.id.localeCompare(right.id);
  });
}

export function upsertWorkbenchGenerationSnapshot<T extends WorkbenchGenerationSnapshot>(
  items: readonly T[],
  next: T,
): T[] {
  return sortWorkbenchGenerationSnapshots([...items.filter((item) => item.id !== next.id), next]);
}

export function latestWorkbenchGenerationSnapshot<T extends WorkbenchGenerationSnapshot>(
  items: readonly T[],
): T | null {
  return sortWorkbenchGenerationSnapshots(items).at(-1) ?? null;
}

export function isWorkbenchGenerationActive(status: WorkbenchGenerationStatus): boolean {
...
export function workbenchGenerationResultStatus(
  ...
  if (status === 'success' || status === 'succeeded' || status === 'partial') return 'success';
  if (status === 'cancelled') return 'cancelled';
  if (['failed', 'rejected', 'expired'].includes(status)) return 'failed';
  return 'pending';
}

export function workbenchGenerationStatusLabel(status: WorkbenchGenerationStatus): string {
  switch (status) {
    case 'pending':
