...
function writeIds(key: string, eventName: string, sessionId: string, enabled: boolean): string[] {
...
export function setSessionPinned(sessionId: string, pinned: boolean): string[] {
  return writeIds(PINNED_SESSIONS_KEY, SESSION_PINS_CHANGED_EVENT, sessionId, pinned);
}
...
export function setSessionUnread(sessionId: string, unread: boolean): string[] {
  return writeIds(UNREAD_SESSIONS_KEY, SESSION_UNREAD_CHANGED_EVENT, sessionId, unread);
}
