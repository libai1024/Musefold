...
const sources = [
  {
    name: 'workbench',
    source: path.join(assetsRoot, 'screens/workbench.png'),
    outputDir: path.join(assetsRoot, 'screens'),
  },
  {
    name: 'library',
    source: path.join(assetsRoot, 'screens/library.png'),
    outputDir: path.join(assetsRoot, 'screens'),
  },
  {
    name: 'floating-library',
    source: path.join(assetsRoot, 'works/floating-library.png'),
    outputDir: path.join(assetsRoot, 'works'),
  },
  {
    name: 'away-from-agent-loop',
    source: path.join(assetsRoot, 'works/away-from-agent-loop.png'),
    outputDir: path.join(assetsRoot, 'works'),
  },
];
...
async function encodeWebp(browser, sourceData, output, width) {
  ...
  try {
    const dataUrl = await page.evaluate(
      async ({ sourceData: imageSource, width: targetWidth }) => {
...
try {
  for (const asset of sources) {
    ...
    for (const width of widths) {
      ...
      execFileSync('sips', ['--resampleWidth', String(width), asset.source, '--out', resizedPng], {
        stdio: 'ignore',
      });
      execFileSync('sips', ['-s', 'format', 'avif', resizedPng, '--out', avif], {
        stdio: 'ignore',
      });
      await encodeWebp(browser, sourceData, webp, width);
      console.log(
        `website image variants: ${asset.name}-${width}.avif + ${asset.name}-${width}.webp`,
      );
    }
  }
} finally {
