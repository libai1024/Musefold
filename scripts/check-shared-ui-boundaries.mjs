...
for (const entry of ['apps/desktop/src/components/layout/AppShell.tsx', 'apps/web/src/App.tsx']) {
  const source = readFileSync(resolve(root, entry), 'utf8');
  if (!source.includes('<ProductSidebarLayout')) {
    violations.push(`${entry}: the complete sidebar rail must use ProductSidebarLayout`);
  }
}
...
for (const entry of [
  ...
  if (!source.includes('<WorkbenchComposerFrame')) {
    violations.push(`${entry}: the complete generation composer must use WorkbenchComposerFrame`);
  }
}

const webStyles = readFileSync(resolve(root, 'apps/web/src/styles.css'), 'utf8');
if (/(^|\n)\.sidebar\s*\{/.test(webStyles)) {
  violations.push('apps/web/src/styles.css: do not reintroduce a host-owned sidebar rail');
}
