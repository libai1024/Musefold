...
if (mode === 'compare') {
  ...
  const files = readdirSync(dirA).filter((f) => f.endsWith('.png') && existsSync(join(dirB, f)));
  if (files.length === 0) {
  ...
  const missing = readdirSync(dirA).filter((f) => f.endsWith('.png') && !existsSync(join(dirB, f)));
