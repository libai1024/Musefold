...
async function verifyHttpBoundary() {
  ...
  assert(unauthorized.headers.get("www-authenticate")?.includes("resource_metadata"),
...
async function discoverConnection() {
  ...
  assert(JSON.stringify(connection.scopes?.sort()) === JSON.stringify(scopes.slice().sort()),
...
async function verifyTools(accessToken) {
  ...
  assert(account.budget && typeof account.officialModelsAvailable === "boolean", "get_account_status omitted capability or budget");
  assert(!/password|access_token|refresh_token|serverUrl|apiKey|secret/i.test(JSON.stringify(account)), "get_account_status exposed sensitive fields");
...
async function cleanupStep(errors, label, operation) {
  try { await operation(); }
  catch (error) { errors.push(new Error(`${label}: ${message(error)}`)); }
}
...
function assertToolCatalog(tools, label) {
  ...
  assert(!actual.some((name) => forbiddenTools.includes(name)),
...
async function request(url, init = {}, options = {}) {
  ...
  const response = await fetch(target, { ...init, headers, redirect: options.redirect ?? "manual" });
...
function parseBaseUrl(value) {
  ...
  catch { console.error("[mcp-full] FAIL MUSEFOLD_STAGING_BASE_URL must be an HTTP(S) URL"); process.exit(2); }
...
function pass(name, detail = "") { console.log(`[mcp-full] PASS ${name}${detail ? ` · ${detail}` : ""}`); }
function skip(name, detail) { console.log(`[mcp-full] SKIP ${name} · ${detail}`); }
...
function createCookieJar() {
  ...
  return {
    ...
    header(url) {
      return [...items.values()].filter((cookie) => url.pathname.startsWith(cookie.path)).map((cookie) => `${cookie.name}=${cookie.value}`).join("; ");
    },
  };
}
