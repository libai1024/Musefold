...
async function authenticate() {
  ...
  assert(
    ...
    'auth response did not include CSRF',
  );
...
async function promptMutationSmoke() {
  ...
  try {
    ...
    const updated = await request(
      `/prompts/${encodeURIComponent(created.id)}`,
      {
        method: 'PATCH',
        body: JSON.stringify({
    ...
    const restored = await request(
      `/prompts/${encodeURIComponent(created.id)}/restore`,
      {
        method: 'POST',
...
async function generationSmoke() {
  ...
  const prompt =
    env.MUSEFOLD_STAGING_GENERATION_PROMPT?.trim() ||
  ...
  assert(
    ['succeeded', 'failed', 'cancelled', 'rejected', 'expired'].includes(job.status),
    'generation did not reach terminal state',
  );
  pass('generation terminal state', job.status);

  const eventsResponse = await fetch(
    `${apiBase}/generations/${encodeURIComponent(first.id)}/events?after=0`,
    {
