...
const archivedSection = readFileSync('src/features/settings/sections/ArchivedChatsSection.tsx', 'utf8');
...
describe('archived chat UI contract', () => {
  it('keeps archived chats as the final settings destination and out of the sidebar header', () => {
    ...
    const archivedIndex = settingsView.indexOf("key: 'archived', label: '已归档聊天'");
  ...
  it('uses exactly the two supported conversation types', () => {
    ...
    expect(sidebar).toContain("data-conversation-kind={c.conversationKind ?? 'chat'}");
  });
