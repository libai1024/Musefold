...
module.exports = {
  forbidden: [
...

    {
      ...
      comment:
        '§3.2 desktop-contracts（当前 shared/）是被依赖的底层：可 import contracts 与 update-protocol（两者同属零 workspace 依赖的纯协议包，处于同一层级；shared 仅 import type Channel，运行时零引入），禁止 import apps/desktop/src/、apps/desktop/electron/ 以及其他 packages/。反向（contracts → shared）由 contracts-no-workspace 覆盖。',
      severity: 'error',
      from: { path: '^shared/' },
      to: {
        path: [
...

    {
      ...
      comment:
        '§3.2 domain 保持 cloud-pure：只依赖 contracts。禁止 shared/（desktop-contracts）、apps/desktop/electron/、apps/desktop/src/、apps/ 以及其他 packages/。window.api 不是模块 import，由 packages-no-desktop-app 拦截通往 IPC 桥的路径。',
      severity: 'error',
...

    {
      ...
      comment:
        '§3.2 core 禁止 electron（桌面主进程无关性）。任务卡：packages/core 不得 import apps/desktop/electron/、apps/desktop/src/。允许 shared/（desktop-contracts 前身）与 contracts。',
      severity: 'error',
