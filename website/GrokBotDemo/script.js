const SHAPES = {
  ...
  pebble:
    'M180 48 C244 48 296 94 300 157 C304 219 263 292 198 304 C132 316 62 276 50 211 C37 143 74 83 132 57 C147 50 163 48 180 48 Z',
  bean: 'M136 54 C205 22 277 64 297 128 C316 188 284 270 225 298 C172 322 86 295 61 232 C37 171 61 91 136 54 Z',
  capsule:
    'M121 50 C84 50 55 78 55 115 L55 245 C55 282 84 310 121 310 L239 310 C276 310 305 282 305 245 L305 115 C305 78 276 50 239 50 Z',
...
const STATES = {
  idle: {
    label: 'Idle',
    readout: 'ready / watching',
    description: 'A quiet baseline. The bot keeps watch and changes expression on a slow rhythm.',
    expressions: ['neutral', 'soft'],
  },
  listening: {
    label: 'Listening',
    readout: 'input / attentive',
    description: 'Eyes open wider and settle toward the next signal.',
    expressions: ['wide', 'soft'],
  },
  thinking: {
    label: 'Thinking',
    readout: 'compute / considering',
    description: 'A slower, focused face for the space between request and answer.',
    expressions: ['focused', 'soft'],
  },
  working: {
    label: 'Working',
    readout: 'runtime / active',
    description: 'The body breathes while a task is in motion.',
    expressions: ['focused', 'wide'],
  },
  happy: {
    label: 'Happy',
    readout: 'result / positive',
    description: 'A short lift after a useful turn, then back to the active state.',
    expressions: ['happy', 'soft'],
  },
  surprised: {
    label: 'Surprised',
    readout: 'event / unexpected',
    description: 'A compact reaction for a new message or a sharp change of context.',
    expressions: ['wide', 'surprised'],
  },
  alerting: {
    label: 'Alerting',
    readout: 'attention / needed',
    description: 'The bot asks for a second look without turning the whole interface red.',
    expressions: ['alert', 'wide'],
  },
};
...
const EXPRESSIONS = {
  neutral: {
    left: [1, 1, 0],
    right: [1, 1, 0],
    mouth: 'M 158 215 Q 180 229 202 215',
    mouthOpacity: 0.66,
  },
  soft: {
    left: [0.9, 0.82, -6],
    right: [0.9, 0.82, 6],
    mouth: 'M 157 216 Q 180 236 203 216',
    mouthOpacity: 0.9,
  },
  focused: {
    left: [0.72, 0.46, -1],
    right: [0.72, 0.46, 1],
    mouth: 'M 162 222 Q 180 218 198 222',
    mouthOpacity: 0.72,
  },
  wide: {
    left: [1.08, 1.14, -2],
    right: [1.08, 1.14, 2],
    mouth: 'M 162 212 Q 180 242 198 212',
    mouthOpacity: 1,
  },
  happy: {
    left: [0.96, 0.56, 0],
    right: [0.96, 0.56, 0],
    mouth: 'M 155 211 Q 180 246 205 211',
    mouthOpacity: 1,
  },
  surprised: {
    left: [1.08, 1.12, 0],
    right: [1.08, 1.12, 0],
    mouth: 'M 180 211 a 15 20 0 1 0 0.1 0',
    mouthOpacity: 1,
  },
  alert: {
    left: [0.85, 0.86, -10],
    right: [0.85, 0.86, 10],
    mouth: 'M 160 230 Q 180 213 200 230',
    mouthOpacity: 0.9,
  },
};
...
function renderButtons() {
  refs.stateOptions.innerHTML = stateKeys
    .map(
      (key) =>
        `<button class="chip${model.state === key ? ' active' : ''}" type="button" data-state="${key}">${STATES[key].label}</button>`,
    )
    .join('');
  refs.shapeOptions.innerHTML = shapeKeys
    .map(
      (key) =>
        `<button class="chip${model.shape === key ? ' active' : ''}" type="button" data-shape="${key}">${key}</button>`,
    )
    .join('');
  refs.stateOptions
    .querySelectorAll('[data-state]')
    .forEach((button) =>
      button.addEventListener('click', () => setState(button.dataset.state, true)),
    );
  refs.shapeOptions
    .querySelectorAll('[data-shape]')
    .forEach((button) => button.addEventListener('click', () => setShape(button.dataset.shape)));
}
...
function renderAvatar() {
  ...
  refs.bodyGroup.setAttribute(
    'transform',
    `translate(180 180) scale(${bodyScale.toFixed(3)} 1) translate(-180 -180)`,
  );
  ...
  refs.botAvatar.classList.toggle(
    'working',
    model.state === 'working' || model.state === 'thinking',
  );
  ...
  refs.statusDot.classList.toggle(
    'busy',
    ['thinking', 'working', 'alerting'].includes(model.state),
  );
...
function advanceExpression() {
  ...
  renderAvatar();
  logEvent(
    `expression → ${STATES[model.state].expressions[model.expressionIndex % STATES[model.state].expressions.length]}`,
  );
}

refs.gazeX.addEventListener('input', (event) => {
  model.gazeX = Number(event.target.value);
  renderAvatar();
});
refs.gazeY.addEventListener('input', (event) => {
  model.gazeY = Number(event.target.value);
  renderAvatar();
});
refs.turnRange.addEventListener('input', (event) => {
  model.turn = Number(event.target.value);
  renderAvatar();
});
...
document.querySelector('#clearLog').addEventListener('click', () => {
  ...
  refs.eventPulse.textContent = 'controller idle';
});
